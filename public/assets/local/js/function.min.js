...
function _response_template(_this, response, meta, back_btn)
{
	...
	else
	{
		...
		else if(!_this.hasClass('--prevent-modify'))
		{
			...
			document.title				= (typeof response.meta !== 'undefined' && typeof response.meta.title !== 'undefined' ? response.meta.title : phrase.error) + ' | ' + config.app_name,
			...
			$(config.content_wrapper).html((typeof response.results !== 'undefined' && typeof response.results.table_data !== 'undefined' ? component.table(response) : (typeof response.results !== 'undefined' && typeof response.results.form_data !== 'undefined' ? ('read' == response.method ? component.view(_this, response) : component.form(_this, response)) : response.html))),
...
function _reactivate(is_index)
{
	...
	if($('input[type=file]').length)
	{
		...
		{
			alert(phrase.this_browser_might_experience_problems_related_to_file_reading_permissions_on_your_device)
		}
	}
		...
		require.js([config.asset_url + 'summernote/summernote.min.js'], function()
		{
			$('[role=wysiwyg]').each(function()
			{
				...
				({
					...
					placeholder: (_this.attr('placeholder') ? _this.attr('placeholder') : phrase.your_content_goes_here),
	...
	if($('[role=colorpicker]').length)
	{
		...
		require.js([config.asset_url + 'colorpicker/colorpicker.min.js'], function()
		{
			...
			{
				if(typeof google !== 'undefined' && typeof polygons !== 'undefined' && polygons.length > 0)
				{
					$.each(polygons, function(key, val)
					{
						val.setOptions
						({
							strokeColor: e.color.toHex(),
							fillColor: e.color.toHex(),
						}),
						google.maps.event.trigger(val, 'dragend')
					})
				}
				
				if(typeof google !== 'undefined' && typeof linestrings !== 'undefined' && linestrings.length > 0)
				{
				...
				if(typeof google !== 'undefined' && typeof markers !== 'undefined' && markers.length > 0)
				{
					$.each(markers, function(key, val)
					{
						val.setIcon
						({
							url: '//chart.apis.google.com/chart?chst=d_map_pin_letter&chld=•|' + e.color.toHex().replace('#', ''),
				...
				if(typeof google !== 'undefined' && typeof draggable_marker !== 'undefined')
				{
					...
					google.maps.event.trigger(draggable_marker, 'dragend')
				}
			})
		})
	}
	...
	if($('[role=uploader]').length)
	{
		$('[role=uploader]').each(function()
		{
			...
			({
				...
				extensions: $(this).hasClass("images") ? ["jpg", "jpeg", "png", "gif", "bmp"] : ["jpg", "jpeg", "png", "gif", "bmp", "pdf", "xls", "xlsx", "doc", "docx", "csv"],
				...
				captions:
				{
					...
					{
						return phrase.choose + ' ' + (options.limit == 1 ? phrase.file : phrase.files)
					},
					feedback: function(options)
					{
						return phrase.choose + ' ' + (options.limit == 1 ? phrase.file : phrase.files) + ' ' + phrase.to_upload
					},
					feedback2: function(options)
					{
						return options.length + ' ' + (options.length > 1 ? ' ' + phrase.files_were : ' ' + phrase.file_was) + ' ' + phrase.chosen
					},
					close: phrase.close,
					download: phrase.download,
					remove: phrase.remove,
					drop: phrase.drag_file_to_upload,
					paste: '<div class="fileuploader-pending-loader"><div class="left-half" style="animation-duration: ${ms}s"></div><div class="spinner" style="animation-duration: ${ms}s"></div><div class="right-half" style="animation-duration: ${ms}s"></div></div>' + phrase.pasting_file,
					removeConfirmation: phrase.are_you_sure_want_to_remove_the_selected_file,
					errors:
					{
						filesLimit: phrase.maximum_file_limit + ' ${limit}.',
						filesType: phrase.allowed_file_type + ' <b>${extensions}</b>.',
						fileSize: '${name} ' + phrase.is_too_large + ' ' + phrase.max_file_size + ' ${fileMaxSize} MB.',
						filesSizeAll: phrase.file_too_large  + ' ' + phrase.max_overall_file_size + ' ${maxSize} MB.',
						fileName: phrase.file_name + ' ${name} ' + phrase.has_been_chosen,
						folderUpload: phrase.folder_upload_is_not_allowed
					}
				}
			})
		})
		...
		$('[role=autocomplete]').each(function()
		{
			...
			require.js([config.asset_url + 'autocomplete/autocomplete.min.js'], function()
			{
				...
				({
					...
					minChars: 3,
					zIndex: 9999999,
					noSuggestionNotice: phrase.nothing_found,
					onSelect: function(suggestion)
					{
						...
						$('*').tooltip('hide');
						if($(this).hasClass('uraian'))
						{
							if(suggestion.disabled)
							{
								$('<div id="modal_alert" class="modal"><div class="modal-dialog modal_table"><div class="modal-content modal_cell animated zoomIn"><div class="row"><div class="col-xs-10 col-xs-offset-1 col-sm-4 col-sm-offset-4"><div class="alert alert-warning" style="border-radius:10px"><div class="row"><div class="col-xs-3"><i class="fa fa-exclamation-triangle fa-4x"></i></div><div class="col-xs-9"><p>Standar harga untuk uraian yang dipilih belum diverifikasi!</p><button type="button" class="btn btn-warning btn-holo btn-sm" data-dismiss="modal"><i class="fa fa-times"></i>Tutup</button></div></div></div></div></div></div></div></div>').appendTo("body").modal();
								$(this).val('');
								$(this).closest('form').find('#nilai_input').val(0);
								$(this).closest('form').find('#satuan_1_input').val('');
								$(this).closest('form').find('#satuan_2_input').val('');
								$(this).closest('form').find('#satuan_3_input').val('');
								return false;
							}
						}
						var	satuan				= $.parseJSON(suggestion.satuan);
						$('*').tooltip('hide');
						(!$('.autocomplete-description').length ? $('<div class="autocomplete-description"></div>').insertAfter($(this).closest('.row')) : null),
						$('.autocomplete-description').html((suggestion.tooltip?'<div class="alert alert-info">' + suggestion.tooltip + '</div>':'') + '<input type="hidden" name="id_standar_harga" class="id_standar_harga_input" value="' + suggestion.id_standar_harga + '" data-original-value="' + suggestion.id_standar_harga + '" />');
						if(!$(this).closest('form').find('#nilai_input').attr('data-original-value'))
						{
							$(this).closest('form').find('#nilai_input').val(suggestion.nilai).trigger('change');
						}
						if($(this).closest('form').find('#satuan_1_input').length)
						{
							$(this).closest('form').find('#satuan_1_input').val(satuan[0]).trigger('change');
						}
						if($(this).closest('form').find('#satuan_2_input').length)
						{
							$(this).closest('form').find('#satuan_2_input').val(satuan[1]).trigger('change');
						}
						if($(this).closest('form').find('#satuan_3_input').length)
						{
							$(this).closest('form').find('#satuan_3_input').val(satuan[2]).trigger('change');
						}
	...
	if($('[role=map]').length)
	{
		$('[role=map]').each(function()
		{
			...
			{
				require.js(['https://maps.googleapis.com/maps/api/js?v=3.0&key=' + config.google_maps_api_key + '&libraries=drawing,geometry,places&language=' + config.language, config.asset_url + 'local/js/google.maps.min.js'], function()
				{
	...
	/**
	 * matching the security hash
	 */
	if($('.--match-security-hash').length && is_index)
	{
		$('.--match-security-hash').each(function()
		{
			var _this					= $(this);
			
			/* check if targeted url has query string */
			if(!Object.keys(get_query_string(null, _this.attr('href'))).length) return;
			...
			(
				...
				function(response)
				{
					if(response.callback !== 'undefined')
					{
						...
						_this.attr('href', response.callback);
					}
				}
			)
		})
	...
	/**
	 * activate additional reactivation function
	 */
