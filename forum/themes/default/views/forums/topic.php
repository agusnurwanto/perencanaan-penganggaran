    ...
    <div class="topic-box">
        ...
        <div class="btn-group btn-group-justified mt vote-con" role="group" data-postid="<?= $topic->id; ?>" data-score="<?= $topic->votes; ?>" data-cast="<?= !empty($vote) ? 1 : 0; ?>">
            <div class="btn-group" role="group">
                <button type="button" data-action="up" class="btn btn-success vote vote-up"<?= (!$loggedIn || $vote && (!$Settings->change_vote || $vote == 1)) ? ' disabled="disabled"' : ''; ?>><i class="fa fa-thumbs-o-up"></i> <span class="total-up"><?= $thread_votes['up'] ? $thread_votes['up'] : 0; ?></span>  <span class="hidden-xs"><?= lang('up_vote'); ?></span></button>
            </div>
            <div class="btn-group" role="group">
                <button type="button" data-action="down" class="btn btn-danger vote vote-down"<?= (!$loggedIn || $vote && (!$Settings->change_vote || $vote == -1)) ? ' disabled="disabled"' : ''; ?>><i class="fa fa-thumbs-o-down"></i> <span class="total-down"><?= $thread_votes['down'] ? $thread_votes['down'] : 0; ?></span> <span class="hidden-xs"><?= lang('down_vote'); ?></span></button>
            </div>
        </div>
