...
class Auth_model extends CI_Model {
    ...
    if ($code !== FALSE) {
        ...
        'active' => ($this->Settings->mode == 1 ? -1 : 1)
        );
    ...
    public function regenrate_code($id = NULL) {
    ...
    public function deactivate($id = NULL, $msg = TRUE) {
    ...
    public function register($username, $password, $email, $additional_data = array(), $active = FALSE) {
        ...
        $data = array(
            ...
            'created_on' => time(),
            'last_login' => time(),
            'active' => ($active ? 1 : (($manual_activation === false) ? 1 : -2))
        );
    ...
    public function login($identity, $password, $remember = FALSE) {
        ...
        $query = $this->db->select($this->identity_column . ', username, email, id, password, active, last_login, last_ip_address, avatar, gender, group_id, avatar_url, message, unban_date')
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($password === TRUE) {
                ...
                if ($user->active < 1) {
                    $this->trigger_events('post_login_unsuccessful');
                    if ($user->active == -3 && !empty($user->message)) {
                        $this->session->set_flashdata('error', sprintf(lang('user_is_banned'), $this->tec->hrsd($user->unban_date), $user->message));
                    } else {
                        $this->set_error('login_unsuccessful_not_active');
                    }
                    return FALSE;
                }
                // check and delete pervious sessions
    ...
    public function delete_user($id) {
        ...
        // delete user records (topics, posts, messages etc) too
        $this->db->delete('topics', array('created_by' => $id));
        ...
        $this->db->delete('votes', array('user_id' => $id));
        $this->db->delete('star_votes', array('user_id' => $id));
        $this->db->delete('user_logins', array('user_id' => $id));
        $this->db->update('conversations', array('sender_delete' => 1), array('sender_id' => $id));
        $this->db->update('conversations', array('receiver_delete' => 1), array('receiver_id' => $id));
        $this->db->delete('api_keys', array('user_id' => $id));
    ...
    public function login_remembered_user() {
        ...
        //get the user
        $this->trigger_events('extra_where');
        $query = $this->db->select($this->identity_column . ', id, username, email, last_login, last_ip_address, avatar, gender, group_id, avatar_url')
    ...
    public function getBadgeByID($id) {
        $q = $this->db->get_where('badges', array('id' => $id));
        if( $q->num_rows() > 0 ) {
            return $q->row();
        }
        return FALSE;
    ...
    public function updateBadge($id, $data) {
        if ($this->db->update('badges', $data, array('id' => $id))) {
            return true;
        }
        return false;
    }

    public function assignBadge($data) {
        if ($this->db->insert('user_badges', $data)) {
            return true;
        }
        return false;
    }
    ...
    public function deleteUserBadge($id) {
        if ($this->db->delete('user_badges', array('id' => $id))) {
            return true;
        }
        return false;
    }

    public function total_users($active = NULL, $query = NULL) {
        if ($query) {
            $this->db->group_start()
            ->like('username', $query, 'both')
            ->or_like('first_name', $query, 'both')
            ->or_like('last_name', $query, 'both')
            ->or_like('email', $query, 'both')
            ->group_end();
        }
        if ($active) {
    ...
    public function get_users($limit, $start, $active = NULL, $query = NULL) {
        ...
        $this->db->limit($limit, $start)->order_by('users.id', 'asc');
        $q = $this->db->get('users');
        if ($q->num_rows() > 0) {
    ...
    public function deleteBadge($id) {
        if ($this->db->delete('badges', array('id' => $id)) && $this->db->delete('user_badges', array('badge_id' => $id))) {
            return true;
        }
        return false;
    }
    ...
    public function getApiKey($user_id) {
        $q = $this->db->get_where('api_keys', array('user_id' => $user_id), 1);
        if( $q->num_rows() > 0 ) {
            return $q->row();
        }
        return FALSE;
