...
class Forums extends MY_Controller
{
	...
	{
		if(!isset($_SERVER['HTTP_COOKIE'])) return false;
		
		$session							= file_get_contents
		(
			str_replace('forum/', null, site_url()) . 'auth/integrator',
			false,
		...
		if($session->username)
		{
			if(!$this->db->get_where('users', array('username' => $session->username))->row())
			{
				$this->db->insert
				(
					'users',
