...
class Verifikasi extends Aksara
{
	...
	public function index()
	{
		...
		(
			...
			(
				...
				'id_prioritas_pembangunan'			=> ('create' == $this->_method ? null : 'disabled'),
				...
				'nama_kegiatan'						=> 'textarea' . ('create' == $this->_method ? null : ', disabled'),
				'images'							=> 'images',
	...
	{
		$this->form_validation->set_rules('alasan_kelurahan', 'Alasan Kelurahan', 'required|min_length[10]');
		...
		(
			...
			'variabel_kelurahan'					=> json_encode($this->input->post('variabel_kelurahan')),
			'nilai_kelurahan'						=> str_replace(',', '', $this->input->post('nilai_kelurahan')),
			'prioritas_kelurahan'					=> $this->input->post('prioritas_kelurahan'),
			'id_prioritas_pembangunan'				=> $this->input->post('id_prioritas_pembangunan'),
			'alasan_kelurahan'						=> $this->input->post('alasan_kelurahan')
	...
	{
		$output										= null;
		$musrenbang									= $this->model->select('survey, rate')->get_where('ta__musrenbang', array('id' => $this->_primary), 1)->row();
		if(!isset($musrenbang->survey)) return false;
		$survey										= json_decode($musrenbang->survey, true);
		if($survey)
		{
			foreach($survey as $key => $val)
			{
				$pertanyaan							= $this->model->select('kode, pertanyaan')->get_where('ref__musrenbang_pertanyaan', array('id' => $key), 1)->row();
				$output								.= '
					<li seq="' . (isset($pertanyaan->kode) ? $pertanyaan->kode : null) . '">
						' . (isset($pertanyaan->pertanyaan) ? $pertanyaan->pertanyaan : null) . '
							...
							<div class="info-box bg-default no-margin">
								...
								<div class="info-box-content">
									<span class="info-box-number">
										<b class="text-primary pull-right">
											' . $musrenbang->rate . '%
										</b>
										Hasil Survey
									</span>
