...
class Tanggapan_kak extends Aksara
{
	...
	public function index()
	{
		...
		{
			return $this->_notification($this->input->get('sub_unit'));
		}
		...
		elseif($this->input->post('asistensi'))
		{
			...
			{
				if($this->model->update('ta__asistensi_kak', array('tanggapan' => $this->input->post('tanggapan'), 'tanggal_tanggapan' => date('Y-m-d H:i:s')), array('id' => $this->input->post('asistensi'))))
				{
					return make_json
					(
						array
		...
		$this->set_title('Tanggapan Asistensi KAK')
		->unset_column('id, id_keg_sub, jenis, sub_jenis, sub_jenis_id, sub_sub_jenis_id, sub_sub_sub_jenis_id, id_operator, operator, dibaca, komentar_dibaca, tanggapan_dibaca, tanggal_tanggapan')
	...
	private function _notification($sub_unit = 0)
	{
		...
		{
			$query									= $this->model->update('ta__asistensi_kak', array('approve' => ('accept' == $this->input->post('action') ? 1 : 0), 'dilihat' => 1, 'alasan' => $this->input->post('reason')), array('id' => $this->input->post('item')), 1);
			if($query)
			{
				return make_json
				(
		...
		(
			'ta__kegiatan_sub',
			'ta__kegiatan_sub.id = ta__asistensi_kak.id_keg_sub'
		)
		->join
		(
			'ta__kegiatan',
		...
		if(in_array($this->session->userdata('group_id'), array(11, 12)) && $this->_sub_unit)
		{
			$this->model->join('ref__sub', 'ref__sub.id = ta__program.id_sub')
			->where('ref__sub.id_unit', $this->_sub_unit);
		}
		
		if($this->input->get('id_keg_sub'))
		{
			$this->model->where('ta__asistensi_kak.id_keg_sub', $this->input->get('id_keg_sub'));
		}
		
		if($this->input->post('counting'))
		{
			$this->model->where('ta__asistensi_kak.tanggapan', '');
		}
		...
		')
		->group_by('id_keg_sub')
		->where('ta__asistensi_kak.comments !=', '')
		...
		if(!$this->input->post('counting'))
		{
			...
			if(in_array($this->session->userdata('group_id'), array(1, 2, 3, 16, 17, 18, 19, 20, 21)) && $sub_unit)
			{
				$this->model->where('ta__program.id_sub', $sub_unit);
			}
			...
			if(in_array($this->session->userdata('group_id'), array(11, 12)) && $this->_sub_unit)
			{
				$this->model->where('ref__sub.id_unit', $this->_sub_unit);
			}
			...
			if($query)
			{
				...
				{
					$counts							= $this->model->select
					('
						count(ta__asistensi_kak.tanggapan) as tanggapan
					')
					->join
					(
						'ta__kegiatan_sub',
					...
					(
						...
						(
							...
							'ta__asistensi_kak.id_keg_sub'	=> $val->id,
						...
						<div class="panel panel-default">
							<div class="panel-heading" role="tab" id="heading_' . $key . '">
								<div class="btn-group-vertical float-right">
									<a href="' . go_to('../../../renja/kegiatan/cetak_kak', array('id_keg_sub' => $val->id, 'method' => 'print')) . '" class="btn btn-success btn-xs" target="_blank" data-toggle="tooltip" title="Cetak KAK">
										<i class="fa fa-print"></i>
										Cetak KAK
									</a>
									<a href="' . go_to('../../../laporan/anggaran/lembar_kak', array('kegiatan' => $val->id, 'method' => 'print')) . '" class="btn btn-warning btn-xs" target="_blank" data-toggle="tooltip" title="Cetak Lembar KAK">
										<i class="fa fa-print"></i>
										Lembar KAK
									</a>
								</div>
							...
							<div class="panel-collapse collapse' . ($this->input->get('id_keg_sub') == $val->id ? ' in' : null) . '" id="accordion_' . $val->id . '" role="tabpanel" aria-labelledby="heading_' . $val->id . '">
	...
	private function _get_thread($id_sub = 0, $id_keg_sub = 0)
	{
		$query										= $this->model->select
		('
			ta__asistensi_kak.*,
		...
		(
			'app__users',
			'app__users.user_id = ta__asistensi_kak.id_operator'
		)
		//->group_by('jenis, id_jenis, jenis_indikator') ? group untuk apa ya?
		->order_by('ta__asistensi_kak.tanggal', 'DESC')
		->get_where
		(
			'ta__asistensi_kak',
			array
			(
				...
				'ta__asistensi_kak.id_keg_sub'		=> $id_keg_sub,
				//'ta__asistensi_kak.tanggapan'		=> ''
			)
		)
		->result();
	...
	private function _filter()
	{
		if(in_array($this->session->userdata('group_id'), array(11, 12)) && $this->_sub_unit)
		{
			$this->model->where('ref__unit.id', $this->_sub_unit);
		}
		$query										= $this->model
		->select
		('
