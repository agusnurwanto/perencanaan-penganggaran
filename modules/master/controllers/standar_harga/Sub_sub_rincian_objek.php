 ...
 */
class Sub_sub_rincian_objek extends Aksara
{
	private $_table									= 'ref__standar_harga_7';
	...
	public function __construct()
	{
		...
		$this->_primary								= $this->input->get('sub_rincian_objek');
	...
	public function index()
	{
		if($this->_primary)
		{
			...
			('
				...
				ref__standar_harga_6.id,
				ref__standar_harga_6.kd_standar_harga_6,
				ref__standar_harga_6.uraian AS sub_rincian_objek
			')
			->join
			(
			...
			(
				...
				array
				(
					'ref__standar_harga_6.id'	=> $this->_primary
				),
				1
			)
			->row();
			...
			if($query)
			{
				...
				('
					...
					<div class="row text-sm border-bottom">
						...
						<label class="col-2 col-sm-1">
							' . $query->kd_standar_harga_1 . '.' . $query->kd_standar_harga_2 . '.' . $query->kd_standar_harga_3 . '.' . $query->kd_standar_harga_4 . '.' . $query->kd_standar_harga_5 . '.' . $query->kd_standar_harga_6 . '
						</label>
						<label class="col-4 col-sm-3">
							' . $query->sub_rincian_objek . '
						</label>
					</div>
				')
				->where('id_standar_harga_6', $query->id)
				->set_default('id_standar_harga_6', $query->id)
				;
			}
			$this
			->unset_column('id, id_standar_harga_6, tahun')
			->unset_field('id, id_standar_harga_6, tahun')
			...
			->join('ref__standar_harga_6', 'ref__standar_harga_6.id = ref__standar_harga_7.id_standar_harga_6')
		...
		else
		{
			...
			(
				'id_standar_harga_6',
				'ref__standar_harga_6.id',
				'{ref__standar_harga_1.kd_standar_harga_1}.{ref__standar_harga_2.kd_standar_harga_2}.{ref__standar_harga_3.kd_standar_harga_3}.{ref__standar_harga_4.kd_standar_harga_4}.{ref__standar_harga_5.kd_standar_harga_5}.{ref__standar_harga_6.kd_standar_harga_6} - {ref__standar_harga_6.uraian AS sub_rincian_objek}',
				array
				(
					'ref__standar_harga_6.tahun'	=> get_userdata('year')
				),
				array
				(
					array
			...
			->set_validation('id_standar_harga_6', 'required|numeric')
		
			->unset_column('id, id_standar_harga_6, rincian_objek, tahun')
			->unset_field('id, tahun')
			;
		}
		
		$this->set_title('Standar Sub Sub Rincian Objek')
		...
		(
			...
			(
				...
				'../sub_rincian_objek'				=> 'Sub Rincian Objek'
			)
		)
		->unset_view('id, tahun')
		...
		->field_order('id_standar_harga_6, kd_standar_harga_7, uraian')
		->set_field
		(
			array
			(
		...
		->set_field('uraian', 'hyperlink', 'standar/harga', array('sub_sub_rincian_objek' => 'id'))
		...
		(
			array
			(
				'id_standar_harga_6'		=> 'get-last-insert',
				'uraian'					=> 'autofocus'
			)
		)
		
		/* pengelompokan konten dalam satu field */
		->merge_content('{kd_standar_harga_1}.{kd_standar_harga_2}.{kd_standar_harga_3}.{kd_standar_harga_4}.{kd_standar_harga_5}.{kd_standar_harga_6}.{kd_standar_harga_7}', 'Kode')
		...
		(
			array
			(
				'kd_standar_harga_7'			=> 'required|numeric|is_unique[ref__standar_harga_6.kd_standar_harga_6.id.' . $this->input->get('id') . '.id_standar_harga_5.' . ($this->_primary ? $this->_primary : $this->input->post('id_standar_harga_5')) . ']',
		...
		(
			array
			(
				'id_standar_harga_6'			=> 'Rincian Objek'
			)
		)
	...
	{
		$id_standar_harga_6							= null;
		if($this->input->get('id'))
		{
			$id_standar_harga_6						= $this->model->select('id_standar_harga_6')->get_where('ref__standar_harga_7', array('id' => $this->input->get('id')), 1)->row('id_standar_harga_6');
		}
		
		$query										= $this->model->select_max('kd_standar_harga_7')->get_where('ref__standar_harga_7', array('id_standar_harga_6' => $this->input->post('primary')), 1)->row('kd_standar_harga_7');
		
		if($query)
		{
			if($id_standar_harga_6 != $this->input->post('primary'))
			{
				$query								= $query + 1;
			}
		}
	...
	private function _filter()
	{
		...
		if($query)
		{
			$option									= '<option value="all">Semua Sub Rincian Objek</option>';
			foreach($query as $key => $val)
			{
				$option								.= '<option value="' . $val->id . '"' . ($val->id == $this->_primary ? ' selected' : null) . '>' . $val->kd_standar_harga_1 . '.' . $val->kd_standar_harga_2 . '.' . $val->kd_standar_harga_3 . '.' . $val->kd_standar_harga_4 . '.' . $val->kd_standar_harga_5 . '.' . $val->kd_standar_harga_6 . ' - ' . $val->sub_rincian_objek . '</option>';
			}
			
			return '
				<select name="sub_rincian_objek" class="form-control" placeholder="Berdasar Sub Rincian Objek">
					' . $option . '
				</select>
			';
		}
