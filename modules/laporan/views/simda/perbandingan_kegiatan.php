			...
			<tbody>
				<?php
					...
					foreach($results as $key => $val)
					{
						...
						$kegiatan					= ((isset($val['kd_keg']) && isset($val['kd_keg_simda']) && $val['kd_keg'] != $val['kd_keg_simda']) || (!isset($val['kd_keg']) || !isset($val['kd_keg_simda'])) ? false : true);
						...
						if(isset($val['kd_urusan']) && $val['kd_urusan'] != $kd_urusan || isset($val['kd_urusan_simda']) && $val['kd_urusan_simda'] != $kd_urusan && isset($val['kd_bidang']) && $val['kd_bidang'] != $kd_bidang || isset($val['kd_bidang_simda']) && $val['kd_bidang_simda'] != $kd_bidang && isset($val['kd_unit']) && $val['kd_unit'] != $kd_unit || isset($val['kd_unit_simda']) && $val['kd_unit_simda'] != $kd_unit && isset($val['kd_sub']) && $val['kd_sub'] != $kd_sub || isset($val['kd_sub_simda']) && $val['kd_sub_simda'] != $kd_sub && isset($val['kd_program']) && $val['kd_program'] != $kd_program || isset($val['kd_program_simda']) && $val['kd_program_simda'] != $kd_program && isset($val['kd_keg']) && $val['kd_keg'] != $kd_keg || isset($val['kd_keg_simda']) && $val['kd_keg_simda'] != $kd_keg)
						{
							echo '
								<tr' . (!$urusan || !$bidang || !$unit || !$sub || !$program || !$id_prog || !$kegiatan || (!isset($val['kegiatan']) || !isset($val['kegiatan_simda'])) || $val['kegiatan'] != $val['kegiatan_simda'] ? ' bgcolor="#ffcccc"' : null) . '>
									<td class="bordered">
										' . (isset($val['kd_urusan']) ? $val['kd_urusan'] : null) . (isset($val['kd_bidang']) ? '.' . $val['kd_bidang'] : null) . (isset($val['kd_unit']) ? '.' . $val['kd_unit'] : null) . (isset($val['kd_sub']) ? '.' . sprintf('%02d', $val['kd_sub']) : null) . (isset($val['kd_program']) ? '.' . sprintf('%02d', $val['kd_program']) : null) . (isset($val['kd_id_prog']) ? '.' . sprintf('%02d', $val['kd_id_prog']) : null) . (isset($val['kd_keg']) ? '.' . sprintf('%02d', $val['kd_keg']) : null) . '
									</td>
									<td style="padding-left:25px" class="bordered">
										' . (isset($val['kegiatan']) ? $val['kegiatan'] : null) . '
									</td>
									<td class="bordered">
										' . (isset($val['kd_urusan_simda']) ? $val['kd_urusan_simda'] : null) . (isset($val['kd_bidang_simda']) ? '.' . $val['kd_bidang_simda'] : null) . (isset($val['kd_unit_simda']) ? '.' . $val['kd_unit_simda'] : null) . (isset($val['kd_sub_simda']) ? '.' . sprintf('%02d', $val['kd_sub_simda']) : null) . (isset($val['kd_program_simda']) ? '.' . sprintf('%02d', $val['kd_program_simda']) : null) . (isset($val['kd_id_prog_simda']) ? '.' . sprintf('%02d', $val['kd_id_prog_simda']) : null) . (isset($val['kd_keg_simda']) ? '.' . sprintf('%02d', $val['kd_keg_simda']) : null) . '
									</td>
									<td style="padding-left:25px" class="bordered">
										' . (isset($val['kegiatan_simda']) ? $val['kegiatan_simda'] : null) . '
									</td>
								</tr>
							';
						}
						...
						$kd_keg						= (isset($val['kd_keg']) ? $val['kd_keg'] : (isset($val['kd_keg_simda']) ? $val['kd_keg_simda'] : 0));
					}
				?>
			</tbody>
		</table>
