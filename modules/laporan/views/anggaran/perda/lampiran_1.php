			...
			<thead>
				<tr>
					...
					<th class="border no-border-left" align="center">
						...
						<h4>
							RINGKASAN APBD YANG DIKLASIFIKASI MENURUT KELOMPOK DAN JENIS
							<br />
		...
		<table class="table">
			<thead>
				<tr>
					...
					<th class="border text-sm-2" width="65%">
						URAIAN
					</th>
			...
			<tbody>
					...
					foreach($results['pendapatan'] as $key => $val)
					{
						...
						if($val->id_rek_3 != $id_rek_3)
						{
							echo '
								<tr>
									<td class="border text-sm">
										' . $val->kd_rek_1 . ' . ' . $val->kd_rek_2 . ' . ' . sprintf('%02d', $val->kd_rek_3) . '
									</td>
									<td style="padding-left:20px" class="border text-sm">
										' . $val->nm_rek_3 . '
									</td>
									<td class="border text-sm text-right">
										' . number_format_indo($val->jumlah_rek_3, 2) . '
									</td>
								</tr>
							';
						}
						if($val->kd_rek_1 == 4)
						{
							$jumlah_pendapatan		+= $val->jumlah_rek_3;
						}
				...
				<?php
					...
					foreach($results['belanja'] as $key => $val)
					{
						...
						if($val->kd_rek_1 == 5)
						{
							$jumlah_belanja		+= $val->jumlah_rek_3;
						}
						$id_rek_1					= $val->id_rek_1;
						$id_rek_2					= $val->id_rek_2;
						$id_rek_3					= $val->id_rek_3;
				...
				<?php
					...
					foreach($results['pembiayaan_penerimaan'] as $key => $val)
					{
						...
						if($val->kd_rek_1 == 6)
						{
							$jumlah_pembiayaan_penerimaan		+= $val->jumlah_rek_3;
						}
						$id_rek_1					= $val->id_rek_1;
						$id_rek_2					= $val->id_rek_2;
						$id_rek_3					= $val->id_rek_3;
				...
				<?php
					...
					foreach($results['pembiayaan_pengeluaran'] as $key => $val)
					{
						...
						if($val->kd_rek_1 == 6)
						{
							$jumlah_pembiayaan_pengeluaran		+= $val->jumlah_rek_3;
						}
						$id_rek_1					= $val->id_rek_1;
						$id_rek_2					= $val->id_rek_2;
						$id_rek_3					= $val->id_rek_3;
