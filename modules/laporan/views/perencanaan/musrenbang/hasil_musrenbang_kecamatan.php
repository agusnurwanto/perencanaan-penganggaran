			...
			<tbody>
				<?php
					...
					foreach($results['data'] as $key => $val)
					{
						...
						$variabel						= json_decode($variabel);
						$variabel_output				= null;
						if($variabel)
						{
							foreach($variabel as $var_key => $var_val)
							{
								$var					= $this->model->select('nama_variabel, satuan')->model->get_where('ref__musrenbang_variabel', array('id' => $var_key), 1)->row();
								if($var)
								{
									$variabel_output	.= ($variabel_output ? ' x ' : null) . $var->nama_variabel . ' ' . $var_val . ' ' . $var->satuan;
								}
							}
						}
						echo '
