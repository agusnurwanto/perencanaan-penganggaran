		...
		<table align="center">
			<tr>
				...
				<td align="center" width="100%" colspan="9">
					...
					?>
						LAPORAN ANGGARAN TAHUN <?php echo get_userdata('year'); ?>
					</h4>
					<h4>
					...
					foreach($results['data'] as $key => $val)
					{
						...
						if(sizeof($results['indikator_kegiatan']) > 0)
						{
							...
							foreach($results['indikator_kegiatan'] as $key_ => $val_)
							{
								...
								$list_indikator_kegiatan[]			= '<td class="bordered" style="padding-left:5px">' . '- ' . $val_['tolak_ukur'] . '</td><td class="bordered">' . (fmod($val_['target'], 1) !== 0.00 ? number_format_indo($val_['target'], 2) : number_format_indo($val_['target'], 0)) . ' ' . $val_['satuan'] . '</td>';
								$rowspan++;
							}
						}
						$new_indikator								= null;
