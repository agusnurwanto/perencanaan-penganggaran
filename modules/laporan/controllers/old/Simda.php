...
class Simda extends Aksara
{
	...
	public function __construct()
	{
		...
		$this->load->model('Simda_model', 'report');
	...
	{
		$this->set_title('Laporan SIMDA')
		->set_icon('mdi mdi-chart-bar')
		->set_output('results', $this->_reports())
		->render();
	}
	
	public function perbandingan_organisasi()
	{
		$this->_title								= 'Perbandingan Organisasi';
		$this->_output								= $this->report->perbandingan_organisasi($this->_tahun);
		
		$this->_execute();
	}
	
	public function perbandingan_program()
	{
		if(!$this->_skpd)
		{
			return throw_exception(403, 'Silakan pilih SKPD untuk ' . phrase($this->_request) . ' Laporan Perbandingan Program!', go_to());
		}
		$this->_title								= 'Perbandingan Program';
		$this->_output								= $this->report->perbandingan_program($this->_tahun, $this->_skpd);
		
		$this->_execute();
	}
	
	public function perbandingan_kegiatan()
	{
		if(!$this->_skpd)
		{
			return throw_exception(403, 'Silakan pilih SKPD untuk ' . phrase($this->_request) . ' Laporan Perbandingan Kegiatan!', go_to());
		}
		$this->_title								= 'Perbandingan Kegiatan';
		$this->_output								= $this->report->perbandingan_kegiatan($this->_tahun, $this->_skpd);
		
		$this->_execute();
	}
	
	public function perbandingan_rekening()
	{
		$this->_title								= 'Perbandingan Rekening';
		$this->_output								= $this->report->perbandingan_rekening($this->_tahun, $this->_skpd);
	...
	private function _reports()
	{
		...
		(
			...
			(
				//'user_group'						=> array(1),
				'title'								=> 'Perbandingan Organisasi',
				'description'						=> 'Laporan Perbandingan Organisasi',
				'icon'								=> 'mdi-mixer',
				'color'								=> 'bg-danger',
				'placement'							=> 'left',
				'controller'						=> 'perbandingan_organisasi',
				'parameter'							=> array
				(
			...
			(
				...
				'description'						=> 'Laporan Perbandingan Program',
				'icon'								=> 'mdi-chart-bar-stacked',
				'color'								=> 'bg-primary',
				'placement'							=> 'left',
				'controller'						=> 'perbandingan_program',
				'parameter'							=> array
				(
					'skpd'							=> $this->_skpd_all(),
					'tanggal_cetak'					=> $this->_tanggal_cetak()
			...
			(
				//'user_group'						=> array(1),
				'title'								=> 'Perbandingan Kegiatan',
				'description'						=> 'Laporan Perbandingan Kegiatan',
				'icon'								=> 'mdi-chart-gantt',
				'color'								=> 'bg-teal',
				'placement'							=> 'left',
				'controller'						=> 'perbandingan_kegiatan',
				'parameter'							=> array
				(
					'skpd'							=> $this->_skpd_all(),
					'tanggal_cetak'					=> $this->_tanggal_cetak()
			...
			(
				//'user_group'						=> array(1),
				'title'								=> 'Perbandingan Rekening',
				'description'						=> 'Laporan Perbandingan Rekening',
				'icon'								=> 'mdi-speaker-bluetooth',
				'color'								=> 'bg-maroon',
				'placement'							=> 'right',
				'controller'						=> 'perbandingan_rekening',
				'parameter'							=> array
				(
					'tanggal_cetak'					=> $this->_tanggal_cetak()
				)
	...
	{
		$output										= null;
		if(1 == get_userdata('group_id') || get_userdata('group_id') == 9)
		{
			$query									= $this->model
			->select
			('
