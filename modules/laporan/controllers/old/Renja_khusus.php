...
/**
 * Laporan > Renja Khusus
 *
 * @version			2.1.0
 * @author			Aby Dahana
 * @profile			abydahana.github.io
 */
class Renja_khusus extends Aksara
{
	private $_title;
	private $_pageSize;
	private $_output;
	...
	public function __construct()
	{
		...
		$this->load->model('Renja_khusus_model', 'report');
	...
	{
		$this->set_title('Laporan Renja Khusus')
	...
	public function renja_awal()
	{
		/*if(!$this->_dprd)
		{
			generateMessages(403, 'Silakan pilih DPRD ' . phrase($this->_request) . ' Laporan Hasil Reses DPRD!', go_to());
		}*/
		//print_r($this->_judul);exit;
	...
	public function renja_akhir()
	{
		...
		$this->_output								= $this->report->renja_akhir($this->_skpd);
		//$this->wkhtmltopdf->pageSize('13.5in', '8.5in');
		//$this->wkhtmltopdf->pageMargin(15);
		$this->_execute();
	}
	...
	public function rekapitulasi_renja_awal_per_skpd()
	{
		...
		$this->_output								= $this->report->rekapitulasi_renja_awal_per_skpd();
		//$this->wkhtmltopdf->pageSize('13.5in', '8.5in');
		//$this->wkhtmltopdf->pageMargin(15);
		$this->_execute();
	}
	...
	public function ba_desk_renja()
	{
		...
		{
			generateMessages(403, 'Silakan pilih SKPD ' . phrase($this->_request) . ' Laporan Berita Acara Desk Renja!', go_to());
		}
	...
	private function _judul()
	{
		...
		return '
			<div class="form-group">
				...
				<input type="text" name="judul" class="form-control input-sm"  placeholder="Silakan Ganti Judulnya">
			</div>
		';
	}
