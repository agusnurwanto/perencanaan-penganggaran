...
class Sub_kegiatan extends Aksara
{
	...
	function __construct()
	{
		...
		$this->_id_keg								= $this->input->get('id_keg_sub');
	...
	public function index()
	{
		...
		if($this->_id_sub && 'all' != $this->_id_sub)
		{
			...
			(
				array
				(
					'id_keg_sub'						=> $this->_id_keg,
					'tahun'								=> get_userdata('year'),
					
				)
			)
			->join('ta__kegiatan_sub', 'ta__kegiatan_sub.id = ' . $this->_table . '.id_keg_sub')
		...
		$this->set_title('kegiatan_sub' . ' ' . ucwords(strtolower($this->_title)))
		->set_icon('fa fa-check-square-o')
		->column_order('kd_keg, kegiatan_sub, nilai_usulan,jenis_usulan, pagu, pagu_1, pengusul')
		->field_order('id_keg_sub, id_sub, kode, nama, pagu')
		...
		->merge_content('<b>{kd_program}.{kd_keg}</b>', phrase('kode'))
		...
		(
			...
			(
				...
				'id_kel'							=> 'Kelurahan',
				'kd_keg_sub'						=> 'Kode Sub Kegiatan',
				'kegiatan_sub'						=> 'Judul Sub Kegiatan',
				'images'							=> 'Gambar'
			)
		)
		->set_field
		(
			array
			(
				'pagu'								=> (in_array(get_userdata('group_id'), array(1, 9, 13)) ? 'number_format' : 'number_format, readonly'),
				'pagu_1'							=> (in_array(get_userdata('group_id'), array(1, 9, 13)) ? 'number_format' : 'number_format, readonly'),
				'nama'								=> 'textarea'
			)
		)
		->set_field
