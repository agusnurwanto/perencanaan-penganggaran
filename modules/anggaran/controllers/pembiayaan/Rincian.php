...
{
	private $_table									= 'ta__anggaran_pembiayaan_rinci';
	
	function __construct()
	{
		parent::__construct();
		$this->_sub_unit							= $this->input->get('sub_unit');
		$this->_pembiayaan							= $this->input->get('pembiayaan');
		if(!$this->_pembiayaan)
		{
			return throw_exception(301, phrase('silakan memilih Rekening Pembiayaan terlebih dahulu'), go_to('../anggaran/pembiayaan/rekening'));
		}
		elseif(!$this->_sub_unit)
		{
			return throw_exception(301, phrase('silakan memilih Rekening terlebih dahulu'), go_to('../anggaran/pembiayaan/sub_unit'));
		}
	...
	public function index()
	{
		...
		$this->set_title('Rincian Anggaran pembiayaan')
		...
		')
		->unset_view('id, id_anggaran_pembiayaan, tahun, vol_123, satuan_123')
		->unset_field('id, id_anggaran_pembiayaan, vol_123, tahun')
		->merge_content('{vol_123} {satuan_123}', phrase('volume'))
		->unset_column('id, id_anggaran_pembiayaan, tahun, vol_1, vol_2, vol_3, satuan_1, satuan_2, satuan_3, vol_123')
		->unset_truncate('uraian')
		->column_order('kd_anggaran_pembiayaan_rinci, uraian, vol_123, volume, nilai, total')
		->field_order('kd_anggaran_pembiayaan_rinci, uraian, nilai, vol_1, satuan_1, vol_2, satuan_2, vol_3, satuan_3, volume, total, satuan_123')
		->view_order('volume')
		...
		(
			array
			(
				'kd_anggaran_pembiayaan_rinci'		=> 'Kode'
			)
		)
		->set_field
		(
			array
			(
				'kd_anggaran_pembiayaan_rinci'		=> 'last_insert',
				'uraian'							=> 'textarea',
				'nilai'								=> 'price_format',
				'vol_1'								=> 'price_format',
				'vol_2'								=> 'price_format',
		...
		->merge_field('kd_anggaran_pembiayaan_rinci, uraian')
		...
		(
			array
			(
				'kd_anggaran_pembiayaan_rinci'		=> 'col-sm-3',
				'nilai'								=> 'col-sm-4',
				'vol_1'								=> 'col-sm-3',
				'vol_2'								=> 'col-sm-3',
				'vol_3'								=> 'col-sm-3'
		...
		(
			array
			(
				'kd_anggaran_pembiayaan_rinci'		=> 'required|is_unique[' . $this->_table . '.kd_anggaran_pembiayaan_rinci.id.' . $this->input->get('id') . '.id_anggaran_pembiayaan.' . $this->_pembiayaan . ']',
				'uraian'							=> 'required',
		...
		(
			array
			(
				'id_anggaran_pembiayaan'			=> $this->_pembiayaan,
		...
		->order_by('kd_anggaran_pembiayaan_rinci')
		->render($this->_table);
	}
	...
	private function _header()
	{
		...
		('
			...
			WHERE
				ta__anggaran_pembiayaan.id = ' . $this->_pembiayaan . '
			LIMIT 1
		')
		->row();
	...
	private function _total_rekening()
	{
		...
		('
			SELECT
				SUM(ta__anggaran_pembiayaan_rinci.total) AS rekening
			FROM
				ta__anggaran_pembiayaan_rinci
			WHERE
				ta__anggaran_pembiayaan_rinci.id_anggaran_pembiayaan = ' . $this->_pembiayaan . '
			LIMIT 1
		')
		->row('rekening');
		return $query;
