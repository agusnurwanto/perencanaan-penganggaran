...
{
	private $_table									= 'ta__rencana_pendapatan';
	...
	function __construct()
	{
		...
		$this->_primary								= $this->input->get('pendapatan');
		$this->set_permission();
		$this->set_theme('backend');
		$this->insert_on_update_fail();
		$this->parent_module('anggaran/pendapatan/rekening');
	...
	public function index()
	{
		...
		//	->set_template('form', 'form')
			//->set_default('id_anggaran_pendapatan', $this->_primary)
			->set_output
			(
				array
				(
			...
			->where('id_anggaran_pendapatan', $this->_primary)
			->limit(1)
			->modal_size('modal-lg')
			->render($this->_table);
		}
	...
	private function _total()
	{
		return $this->model
		->select_sum('total')
		->get_where('ta__anggaran_pendapatan_rinci', array('ta__anggaran_pendapatan_rinci.id_anggaran_pendapatan' => $this->_primary))
		->row('total');
	}
	...
	private function _validate_form()
	{
		...
		else
		{
			$prepare								= array
			(
				'id_anggaran_pendapatan'			=> $this->_primary,
			...
			$checker								= $this->db->select('id')->where('id_anggaran_pendapatan', $this->_primary)->limit(1)->get($this->_table)->num_rows();
			if($checker > 0)
			{
				$execute							= $this->model->update($this->_table, $prepare, array('id_anggaran_pendapatan' => $this->_primary));
			}
			else
			{
	...
	public function validate_input()
	{
		...
		->select_sum('ta__anggaran_pendapatan_rinci.total', 'total_rekening')
		->get_where('ta__anggaran_pendapatan_rinci', array('ta__anggaran_pendapatan_rinci.id_anggaran_pendapatan' => $this->_primary))
		->row('total_rekening');
