...
class Rekening extends Aksara
{
	...
	public function index()
	{
		...
		');
		if($header->lock_kegiatan_sub == 1 && $header->pilihan == 1)
		{
			$this->unset_action('create, update, delete, pdf, export, print');
		}
		else
		{
			$this->unset_action('pdf, export, print');
		}
		
		/*if('create' == $this->_method)
		{
			$this->set_selected('id_sumber_dana', $header->id_sumber_dana);
		}*/
		...
		->add_action('toolbar', '../../laporan/anggaran/rka/rka_sub_kegiatan', 'RKA Sub Kegiatan', 'btn-info ajax', 'mdi mdi-printer', array('kegiatan' => $this->_kegiatan, 'sub_kegiatan' => $this->_primary, 'method' => 'embed'), true)
		...
		->add_action('option', '../rencana_keuangan', 'Rencana', 'btn-success --modal', 'mdi mdi-cash-usd', array('id_belanja' => 'id'))
		->add_class
		(
			array
			(
		...
		->unset_column('id, id_keg_sub, kode, nama_sumber_dana, tahun, kd_sumber_dana_rek_1, kd_sumber_dana_rek_2, kd_sumber_dana_rek_3, kd_sumber_dana_rek_4, kd_sumber_dana_rek_5')
		->unset_view('id, id_keg_sub, tahun')
		->unset_field('id, id_keg_sub, tahun')
		->unset_truncate('uraian')
		->unset_action('pdf, export, print')
		
		->column_order('kd_rek_1, uraian, nama_sumber_dana')
		->field_order('id_rek_6')
		->merge_content('<b>{kd_rek_1}.{kd_rek_2}.{kd_rek_3}.{kd_rek_4}.{kd_rek_5}.{kd_rek_6}</b>', 'Kode')
		->set_field('uraian', 'hyperlink', 'anggaran/sub', array('belanja' => 'id'))
		->set_alias
		(
			array
			(
		...
		(
			array
			(
				'id_rek_6'							=> 'required|is_unique[' . $this->_table . '.id_rek_6.id.' . $this->input->get('id') . '.id_keg_sub.' . $this->_primary . ']|callback_cek_realisasi' //,
		...
		')
		->join('ta__rencana', 'ta__rencana.id_belanja = ta__belanja.id', 'left')
		->set_field
		(
			array
			(
	...
	private function _anggaran()
	{
		...
		('
			...
			WHERE
				ta__belanja.id_keg_sub = ' . $this->_primary . '
			LIMIT 1
		')
		->row('anggaran');
		return $query;
