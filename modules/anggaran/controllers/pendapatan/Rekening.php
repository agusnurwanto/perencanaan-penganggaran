...
{
	private $_table									= 'ta__anggaran_pendapatan';
	...
	function __construct()
	{
		...
		if(!$this->_sub_unit)
		{
			return throw_exception(301, 'silakan memilih Sub Unit terlebih dahulu', go_to('../../anggaran/pendapatan/sub_unit'));
		}
		$this->set_permission();
	...
	public function index()
	{
		...
		->add_action('option', '../rencana_keuangan', 'Rencana', 'btn-success --modal', 'mdi mdi-cash-usd', array('pendapatan' => 'id'))
		->add_class
		(
			array
			(
		...
		->unset_column('id, kode, id_sub, tahun, kd_sumber_dana_rek_1, kd_sumber_dana_rek_2, kd_sumber_dana_rek_3, kd_sumber_dana_rek_4, kd_sumber_dana_rek_5')
		...
		->set_field('uraian', 'hyperlink', 'anggaran/pendapatan/rincian', array('pendapatan' => 'id'))
		->set_alias
		(
			array
			(
		...
		->select
		('
			(ta__rencana_pendapatan.jan + ta__rencana_pendapatan.feb + ta__rencana_pendapatan.mar) AS tw_1,
			(ta__rencana_pendapatan.apr + ta__rencana_pendapatan.mei + ta__rencana_pendapatan.jun) AS tw_2,
			(ta__rencana_pendapatan.jul + ta__rencana_pendapatan.agt + ta__rencana_pendapatan.sep) AS tw_3,
			(ta__rencana_pendapatan.okt + ta__rencana_pendapatan.nop + ta__rencana_pendapatan.des) AS tw_4
		')
		->join('ta__rencana_pendapatan', 'ta__rencana_pendapatan.id_anggaran_pendapatan = ta__anggaran_pendapatan.id', 'left')
		->set_field
		(
			array
			(
					...
					<tbody>
						<tr>
							...
							<td class="text-sm" width="57%">
								<a href="' . base_url('laporan/anggaran/rka/rekening', array('rekening' => 9, 'method' => 'embed', 'tanggal_cetak' => date('Y-m-d'))) . '" class="btn btn-info btn-sm float-right" target="_blank">
