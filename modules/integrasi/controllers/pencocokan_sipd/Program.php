...
/**
 * Integrasi > SIPD > Pencocokan SIPD > Program
 *
 * @version			1.0.0
 * @author			Aby Dahana
...
class Program extends Aksara
{
	...
	public function index()
	{
		...
		$this->set_title('Pencocokan Program SIPD')
	...
	private function _data()
	{
		...
		->order_by('sipd__ref__urusan.kd_urusan, sipd__ref__bidang.kd_bidang, sipd__ref__program.kd_program')
		->get_where
		(
			'sipd__ref__program',
			array
			(
				'sipd__ref__program.tahun'			=> get_userdata('year')
			)
		)
		->result_array();
		...
		->order_by('ref__urusan.kd_urusan, ref__bidang.kd_bidang, ref__program.kd_program')
		->get_where
		(
			'ref__program',
			array
		...
		{
			$kode									= $val['kd_urusan'] . '.' . sprintf('%02d', $val['kd_bidang']) . '.' . sprintf('%02d', $val['kd_program']);
