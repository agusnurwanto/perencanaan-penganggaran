...
/**
 * Integrasi > SIPD > Pencocokan SIPD > Unit
 *
 * @version			1.0.0
 * @author			Aby Dahana
...
class Unit extends Aksara
{
	private $_table									= 'tmp__unit';
	...
	public function index()
	{
		...
		$this->set_title('Pencocokan Unit SIPD')
	...
	private function _data()
	{
		...
		(
			...
			array
			(
				'sipd__ref__unit.tahun'				=> get_userdata('year')
			)
		)
		->result_array();
		...
		{
			$kode									= $val['kd_urusan'] . '.' . sprintf('%02d', $val['kd_bidang']) . '.' . $val['kd_urusan_2'] . '.' . sprintf('%02d', $val['kd_bidang_2']) . '.' . $val['kd_urusan_3'] . '.' . sprintf('%02d', $val['kd_bidang_3']) . '.' . sprintf('%02d', $val['kd_unit']);
