...
class Referensi_kegiatan extends Aksara
{
	private $_table									= 'tmp__referensi_kegiatan';
	...
	private function _data()
	{
		...
		('
			...
			sipd__ref__kegiatan.nm_kegiatan,
			"sipd" AS source
		')
		->join
		(
		...
		->order_by('sipd__ref__urusan.kd_urusan, sipd__ref__bidang.kd_bidang, sipd__ref__program.kd_program, sipd__ref__kegiatan.kd_kegiatan')
		->get_where
		(
			'sipd__ref__kegiatan',
			array
			(
				'sipd__ref__kegiatan.tahun'			=> get_userdata('year')
			)
		)
		->result_array();
		...
		->order_by('ref__urusan.kd_urusan, ref__bidang.kd_bidang, ref__program.kd_program, ref__kegiatan.kd_kegiatan')
		->get_where
		(
			'ref__kegiatan',
			array
		...
		{
			$kode									= $val['kd_urusan'] . '.' . sprintf('%02d', $val['kd_bidang']) . '.' . sprintf('%02d', $val['kd_program']) . '.' . sprintf('%02d', $val['kd_kegiatan']);
		...
		foreach($query_siencang as $key => $val)
		{
			...
			if(isset($output[$kode]))
			{
				...
				(
					...
					(
						'kode_siencang'				=> $kode,
						'label_siencang'			=> $val['nm_kegiatan']
					),
					array
					(
						'kode_sipd'					=> $kode
