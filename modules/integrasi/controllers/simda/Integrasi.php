...
class Integrasi extends Aksara
{
	function __construct()
	{
		parent::__construct();
		$this->_id_sub								= (5 == get_userdata('group_id') or 11 == get_userdata('group_id') ? get_userdata('sub_unit') : $this->input->get('id_sub'));
		
		if(!in_array(get_userdata('group_id'), array(1, 5, 9, 11, 13)))
		{
	...
	{
		if(get_userdata('group_id') == 1 || get_userdata('group_id') == 9 || get_userdata('group_id') == 13)
		{
		...
		->set_field('nm_sub', 'hyperlink', 'renja/integrasi/per_item', array('id_sub' => 'id'))
