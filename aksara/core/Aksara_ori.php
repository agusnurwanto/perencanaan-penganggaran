...
class Aksara extends CI_Controller
{
	...
	public function _remap($method = null, $params = array())
	{
		...
		else
		{
			...
			list($q, $w, $e, $r, $t, $t, $y, $u)	= array_pad($params, 8, null);
			
			/* push into controller */
			$this->index($q, $w, $e, $r, $t, $t, $y, $u);
		}
	}
	 ...
	 * @access		private
	 */
	private function _get_relation($params = array(), $selected = null)
	{
		$output										= null;
		$replace_me									= array();
		...
		if($params['join'] && $params['group_by'])
		{
			$this->model->group_by($params['group_by']);
		}
		
		$query										= $this->model->get($params['relation_table'], $params['limit'])->result_array();
		
		if($query)
		{
			foreach($query as $key => $val)
			{
				$label								= $params['formatting'];
				$attributes							= (isset($this->_set_attribute[$primary_key]) ? $this->_set_attribute[$primary_key] : null);
				$option_label						= (isset($this->_set_option_label[$primary_key]) ? $this->_set_option_label[$primary_key] : null);
				foreach($params['select'] as $magic => $replace)
				{
					...
					{
						$replacement				= str_replace('.', '', strstr($replace, '.'));
						$is_contain_as				= strtolower($replacement);
						if(stripos(trim($is_contain_as), ' AS ') !== false)
						{
							$replacement			= substr($replacement, strrpos($replacement, ' ') + 1);
						}
					}
		...
		{
			$output									= '
				<select name="' . $primary_key . '" class="form-control' . (isset($this->_add_class[$primary_key]) ? ' ' . $this->_add_class[$primary_key] : null) . '" placeholder="' . phrase('please_choose') . '" id="' . $primary_key . '_input"' . (isset($this->_set_field[$primary_key]['field_type']) && in_array('disabled', $this->_set_field[$primary_key]['field_type']) ? ' disabled' : null) . '>
	...
	public function render($table = null, $view = null)
	{
		...
		if($table && $this->model->table_exists($table))
		{
			...
			if(!$this->_set_primary)
			{
				$this->set_description('<div class="alert alert-danger text-sm mb-0"><b>' . phrase('no_primary_key_found') . '!</b> ' . phrase('please_define_it_manually') . ' (' . phrase('refer_to') . ' <code>set_primary</code>). ' . phrase('otherwise_you_cannot_perform_the_following_action') . ': ' . phrase('read') . ', ' . phrase('update') . ', ' . phrase('delete') . ' ' . phrase('and') . ' ' . phrase('export_to_document') . '</div>');
				...
				if((!$this->_like && $this->input->get('q')) || ('autocomplete' == $this->input->post('method') && $this->input->post('q')))
				{
					...
					else
					{
						...
						if($this->_select)
						{
							...
							{
								if(stripos(trim($like_val), ' AS ') === false)
								{
									$v				= $like_val;
								}
								else
								{
									$v				= stristr($like_val, ' AS ', true);
								}
								if($this->_like)
								{
									$this->_or_like[$v]		= ('autocomplete' == $this->input->post('method') && $this->input->post('q') ? $this->input->post('q') : $this->input->get('q'));
			...
			if($this->input->is_ajax_request() && 'autocomplete' == $this->input->post('method'))
			{
				...
				else
				{
					...
					{
						/* check if the selected field is used alias */
						if(stripos(trim($val), ' AS ') !== false)
						{
							/* eplode to get the original field source */
							$explode				= preg_split('/ AS /i', $val);
							
							/* check if original field source is found */
							if(isset($explode[0]))
							{
								/* set to val */
								$val				= $explode[0];
	...
	public function render_form($data = array())
	{
		...
		if($serialized)
		{
			...
			foreach($serialized as $field => $params)
			{
				...
				{
					$content						= '<textarea name="' . $field . '" class="form-control' . $extra_class . '" role="tagsinput" placeholder="' . phrase('separate_with_comma') . '" id="' . $field . '_input"' . $read_only . ' spellcheck="false">' . $content . '</textarea>';
				}
				...
				{
					$content						= '<input type="text" name="' . $field . '" class="form-control' . $extra_class . '" role="datepicker" data-modal="true" data-large-mode="true" placeholder="' . phrase('click_to_select_data') . '" value="' . ($content && $content != '0000-00-00' ? date('d M Y', strtotime($content)) : date('d M Y')) . '" id="' . $field . '_input" maxlength="' . $max_length . '" readonly' . $read_only . ' spellcheck="false" />';
				}
				elseif(in_array('datetime', $type))
				{
					$content						= '<input type="text" name="' . $field . '" class="form-control' . $extra_class . '" role="datetime" data-modal="true" data-large-mode="true" placeholder="' . phrase('click_to_select_data') . '" value="' . ($content && $content != '0000-00-00 00:00:00' ? $content : date('Y-m-d H:i:s')) . '" id="' . $field . '_input" maxlength="' . $max_length . '"' . $read_only . ' spellcheck="false" />';
				}
				...
				{
					$content						= '<textarea name="' . $field . '" class="form-control' . $extra_class . '" role="wysiwyg" data-upload-path="' . $this->_set_upload_path . '" placeholder="' . phrase('type_the_content_here') . '" id="' . $field . '_input"' . $attribute . ' maxlength="' . $max_length . '" rows="1"' . $read_only . ' spellcheck="false">' . $content . '</textarea>';
				}
				elseif(in_array('textarea', $type))
				{
					$content						= '<textarea name="' . $field . '" class="form-control' . (in_array('autocomplete', $type) && $extra_params ? ' on-autocomplete-trigger' : null) . $extra_class . '" id="' . $field . '_input"' . $attribute . ' maxlength="' . $max_length . '" rows="1"' . (in_array('autocomplete', $type) ? ' role="autocomplete" data-href="' . current_page() . '"': '') . $read_only . ' spellcheck="false">' . $content . '</textarea>';
				}
				elseif(in_array('price_format', $type))
				{
					$content						= '<input type="text" name="' . $field . '" min="0" class="form-control text-right' . $extra_class . '" value="' . ($content ? $content : 0) . '" role="price" placeholder="' . phrase('number_only') . '" id="' . $field . '_input" maxlength="' . $max_length . '"' . $read_only . ' spellcheck="false" />';
				}
				...
				{
					$content						= '<input type="email" name="' . $field . '" class="form-control' . $extra_class . '" placeholder="' . phrase('type_the_valid_email') . '" value="' . $content . '" id="' . $field . '_input" maxlength="' . $max_length . '"' . $read_only . ' />';
				}
				...
				elseif(in_array('last_insert', $type))
				{
					...
					$content						= '<input type="text" name="' . $field . '" class="form-control' . $extra_class . '" value="' . $content . '" id="' . $field . '_input" maxlength="' . $max_length . '"' . $read_only . ' spellcheck="false" />';
				}
				elseif(in_array('to_slug', $type))
				{
					$content						= '<input type="text" name="' . $field . '" class="form-control' . $extra_class . '" value="' . $content . '" placeholder="' . (!$read_only ? phrase('leave_blank_to_generate_automatically') : null) . '" id="' . $field . '_input" maxlength="' . $max_length . '"' . $read_only . ' spellcheck="false" />';
				}
				...
				{
					$content						= '<input type="text" name="' . $field . '" class="form-control' . (in_array('autocomplete', $type) && $extra_params ? ' on-autocomplete-trigger' : null) . $extra_class . '" value="' . $content . '" id="' . $field . '_input"' . $attribute . ' maxlength="' . $max_length . '"' . (in_array('autocomplete', $type) && $extra_params ? ' role="autocomplete" data-href="' . current_page() . '"': '') . $read_only . ' spellcheck="false" />';
				}
	...
	private function _query($table = null)
	{
		...
		if(!in_array($this->_method, array('update', 'delete')) && is_array($this->_select) && sizeof($this->_select) > 0)
		{
			...
			foreach($this->_select as $key => $val)
			{
				...
				elseif(stripos(trim($val), ' AS ') === false)
				{
					/**
					 * Individual table
					 */
					$unset							= explode('.', $val);
					if(isset($unset[0]) && isset($unset[1]) && in_array($unset[1], $this->_forge_select))
					{
						/**
			...
			if(is_array($this->_order_by) && sizeof($this->_order_by) > 0)
			{
				...
				foreach($this->_order_by as $key => $val)
				{
					...
					if($this->model->field_exists($key, $this->_from))
					{
						...
						$this->model->order_by($this->_from . '.' . $key, $val);
					}
					/**
					 * Validate the column to check if column is exist in table
					 */
