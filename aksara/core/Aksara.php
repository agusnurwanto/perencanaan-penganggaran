...
/**
 * Aksara
 * This class is override the codeigniter base controller and add the extra CRUD
 * function that  generate  a magic output.  So you don't  need create model or
 * view anymore.
 *
 * I would give comments in every single  function but i choose live dangerously,
 * so it might implemented in the next decade.
 *
...
class Aksara extends CI_Controller
{
	...
	private $_add_action							= null;
	private $_add_class								= array();
	private $_add_attribute							= array();
	...
	function __construct()
	{
		...
		$this->load->helper(array('global', 'image', 'language', 'url'));
		
		/* set the language based by the browser default language */
		set_user_language();
		
		/* set default language */
		if($this->session->userdata('language') && is_dir(APPPATH . 'language/' . $this->session->userdata('language')))
		{
		...
		if('token' == $this->input->post('request'))
		{
			...
			(
				array
				(
					'token'							=> sha1(current_page() . SALT),
					'callback'						=> current_page(null, array('token' => generate_token($this->input->get())))
				)
			);
		}
		...
		if($this->input->get())
		{
			...
			elseif($this->input->get('code') && $this->input->get('scope') && $this->input->get('session_state') && $this->input->get('prompt') && method_exists($this, 'google_auth'))
			{
			...
			elseif($this->input->get('access_token') && $this->input->get('email') && method_exists($this, 'validate_token'))
			{
				return $this->validate_token($this->input->get('access_token'), $this->input->get('email'));
			}
			...
			/* validate token */
			if($query_string && $token != generate_token($query_string))
			{
	...
	public function _remap($method = null, $params = array())
	{
		...
		else
		{
			...
			/* list the argument, who th used 8 slugs into url? */
	 ...
	 * @params		mixed		$key		extra query string
	 * @return		array
	 */
	public function add_action($placement = 'option', $url = null, $label = null, $class = null, $icon = null, $key = array(), $new_tab = false)
	{
		$this->_add_action[]						= array
		(
			'placement'								=> $placement,
			'url'									=> $url,
			'label'									=> $label,
	...
	/**
	 * add_attr
	 * Add attribute to the rendered field
	 *
	 * NOTE: It's case sensitive!
	 *
	 * @access		public
	 * @return		array
	 */
	public function add_attribute($params = array(), $attribute = null, $value = null)
	{
		if(!is_array($params))
		{
			/* shorthand possibility */
			$params									= array
			(
				$params								=> $attribute . ($value ? '="' . $value . '"' : null)
			);
		}
		$this->_add_attribute						= array_merge($this->_add_attribute, $params);
		return $this;
	}
	 ...
	 * @return		mixed
	 */
	public function set_field($params = array(), $type = array(), $parameter = null, $extra_params = null, $another_params = null, $skip = false, $order = null, $placeholder = null)
	{
		if(!is_array($type))
		{
	 ...
	 * @return		mixed
	 */
	public function unset_delete($params = array(), $value = null)
	{
		if(!is_array($params))
		{
			/* shorthand possibility */
	...
	/**
	 * set_selected
	 * The function to set selected value on form field
	 *
	 * NOTE: It's case sensitive!
	 *
	 * @access		public
	 ...
	 * @return		mixed
	 */
	public function set_selected($params = array(), $value = null)
	{
		if(!is_array($params))
		{
			/* shorthand possibility */
		...
		$this->_set_selected						= array_merge($this->_set_selected, $params);
		return $this;
	}
	
	/**
	 * default_value
	 * The function to set default value on form field
	 *
	 * NOTE: It's case sensitive!
	 *
	 * @access		public
	 ...
	 * @return		mixed
	 */
	public function default_value($params = array(), $value = 0)
	{
		if(!is_array($params))
		{
			/* shorthand possibility */
		...
		$this->_default_value						= array_merge($this->_default_value, $params);
		return $this;
	}
	 ...
	 * @access		private
	 */
	private function _get_relation($params = array(), $selected = null, $limit = null)
	{
		...
		elseif(in_array($this->_method, array('create', 'update')))
		{
			$output									= '
				<select name="' . $primary_key . '" class="form-control' . (isset($this->_add_class[$primary_key]) ? ' ' . $this->_add_class[$primary_key] : null) . '" placeholder="' . phrase('please_choose') . '" id="' . $primary_key . '_input"' . (isset($this->_add_attribute[$primary_key]) ? ' ' . $this->_add_attribute[$primary_key] : null) . (isset($params['limit']) && $params['limit'] > 1 ? ' data-limit="' . $params['limit'] . '" data-href="' . current_page() . '"' : null) . (isset($this->_set_field[$primary_key]['field_type']) && in_array('disabled', $this->_set_field[$primary_key]['field_type']) ? ' disabled' : null) . '>
					<option value="' . (!$is_selected_exist ? $selected : null) . '">' . ($selected ? $this->_get_relation($params, $selected, 1) : phrase('please_choose')) . '</option>
					' . $output . '
				</select>
			';
		}
	...
	public function render($table = null, $view = null)
	{
		...
		if($table && $this->model->table_exists($table))
		{
			...
			if(in_array($this->_method, array('read', 'update', 'delete')) || (in_array($this->_method, array('print', 'pdf')) && $this->input->get('r') == 'document'))
			{
				$this->_limit						= 1;
				$this->_offset						= null;
				...
				if(is_array($this->_set_primary) && sizeof($this->_set_primary) > 0)
				{
					...
					foreach($this->_set_primary as $key => $val)
					{
						...
						{
							if(('read' == $this->_method && isset($this->_unset_read[$val]) && in_array($this->input->get($val), $this->_unset_read[$val])) || ('update' == $this->_method && isset($this->_unset_update[$val]) && in_array($this->input->get($val), $this->_unset_update[$val])) || ('delete' == $this->_method && isset($this->_unset_delete[$val]) && in_array($this->input->get($val), $this->_unset_delete[$val])))
							{
						...
						foreach($val as $field => $value)
						{
							...
							if(!$autocomplete_item && $value['original'])
							{
								...
								(
									...
									'target'		=> current_page(null, array('per_page' => null, 'q' => $this->input->post('q')))
								);
							}
						}
			...
			{
				$this->_set_icon					= 'mdi mdi-file-plus';
				$this->_set_title					= phrase('add_new_data');
				$this->_set_description				= (isset($this->_set_description['create']) ? $this->_set_description['create'] : '<div class="pt-3 pb-3 text-muted">' . phrase('please_fill_all_required_field_below_to_add_new_data') . '</div>');
			...
			elseif('read' == $this->_method)
			{
				...
				$this->_set_description				= (isset($this->_set_description['read']) ? $this->_set_description['read'] : '<div class="pt-3 pb-3 text-muted">' . phrase('showing_the_result_of_the_selected_item') . '</div>');
			...
			elseif('update' == $this->_method)
			{
				...
				$this->_set_description				= (isset($this->_set_description['update']) ? $this->_set_description['update'] : '<div class="pt-3 pb-3 text-muted">' . phrase('make_sure_to_check_the_changes_before_submitting') . '</div>');
				$this->_view						= (isset($this->_set_template['form']) ? $this->_set_template['form'] : ($view && 'index' != $view ? $view : 'form'));
				$this->_results						= $this->render_form($this->_query);
			}
			...
			elseif(in_array($this->_method, array('export', 'print', 'pdf')))
			{
				...
				$this->_results						= ('document' == $this->input->get('r') ? $this->render_read($this->_query) : $this->render_table($this->_query));
			}
		...
		else
		{
			...
			$this->_set_title						= ($this->_set_title ? $this->_set_title : ($this->_query ? phrase('title_was_not_set') : phrase('page_not_found')));
		...
		elseif('pdf' == $this->_method || 'export' == $this->_method)
		{
			/**
			 * Method document
			 */
			...
			$this->document->generate($this->_output, $this->_set_title, 'embed');
		}
		...
		else
		{
			...
			if($this->input->post('token'))
			{
				...
				/* validate sent token */
				$token_sent							= $this->input->post('token');
				if($token_sent == sha1(current_page() . SALT))
				{
			...
			else
			{
				...
				$language							= ($this->session->userdata('language') ? $this->session->userdata('language') : get_setting('app_language'));
				
				/* Display to the browser */
				return $this->template->render($this->_view, $this->_output, $this->_set_breadcrumb, $language);
			}
		}
	}
		...
		if($table && $this->model->table_exists($table))
		{
			if(is_array($where) && sizeof($where) > 0 && $this->model->get_where($table, $where, 1)->num_rows() > 0)
			{
				...
				if($this->model->update($table, $data, $where, 1))
				{
					...
					$data							= $this->render_table($data);
					$output							= array();
					
					if(isset($data['table_data'][0]))
					{
						foreach($data['table_data'][0] as $key => $val)
						{
					...
					$output							= array
					(
						'token'						=> 'item__' . $this->input->get('token'),
						'data'						=> $output
					);
					
					return throw_exception(301, phrase('data_was_successfully_updated'), $this->_redirect_back, $output);
				}
				else
				{
					if($callback && method_exists($this, $callback))
	...
	public function delete_data($table = null, $where = array(), $limit = 1)
	{
		...
		{
			return throw_exception(301, phrase('your_delete_action_has_been_sent_but_will_not_affected_in_demo_mode'), $this->_redirect_back);
		}
		...
		if($table && $this->model->table_exists($table))
		{
			...
			if($this->model->get_where($table, $where, $limit)->num_rows() > 0)
			{
	...
	public function delete_batch($table = null)
	{
		...
		if(is_array($items) && sizeof($items) > 0)
		{
			...
			foreach($items as $key => $val)
			{
				...
				if($this->model->delete($table, $val))
				{
					$affected_rows++;
				}
			}
	...
	public function render_form($data = array())
	{
		...
		if($serialized)
		{
			...
			foreach($serialized as $field => $params)
			{
				...
				$attribute							= (isset($this->_add_attribute[$field]) ? $this->_add_attribute[$field] : null);
					...
					$content						= '
						<div data-provides="fileupload" class="fileupload fileupload-new text-sm-center">
							<span class="btn btn-light btn-file">
								<input type="file" name="' . $field . '" accept="image/*" role="image-upload" id="' . $field . '_input" />
				...
				elseif(in_array('attributes', $type))
				{
								...
								<div class="row mb-1">
									...
									<div class="col-3">
										<div class="btn-group btn-group-sm float-right">
											<a href="javascript:void(0)" class="btn btn-secondary --move-up" data-element=".row" data-toggle="tooltip" title="' . phrase('move_up') . '">
												<i class="mdi mdi-arrow-collapse-up"></i>
											</a>
											<a href="javascript:void(0)" class="btn btn-secondary --move-down" data-element=".row" data-toggle="tooltip" title="' . phrase('move_down') . '">
												<i class="mdi mdi-arrow-collapse-down"></i>
											</a>
											<a href="javascript:void(0)" class="btn btn-secondary" role="remove-attribute" data-element=".row">
					...
					$content						= '
						<div class="attribute-input">
							...
							<div class="row">
								<div class="col-4 pr-0">
									<button type="button" class="btn btn-secondary btn-sm btn-block" role="add-attribute" data-label="' . $field . '[label][]" data-label-placeholder="' . phrase('label') . '" data-value-placeholder="' . phrase('value') . '" data-value="' . $field . '[value][]">
				...
				{
					$carousels						= json_decode($content, true);
					$items							= null;
					if(is_array($carousels) && sizeof($carousels) > 0)
					{
									...
									<div class="card-body">
										<div class="row">
											<div class="col-md-6">
												<div class="form-group">
													<label class="text-muted">
														' . phrase('slider_background') . '
													</label>
													<div data-provides="fileupload" class="fileupload fileupload-new text-sm-center">
														<span class="btn btn-light btn-file btn-block">
															<input type="file" name="' . $field . '[background][]" accept="image/*" role="image-upload" id="' . $field . '_input" />
															<div class="fileupload-new text-center">
											...
											<div class="col-md-6">
												<div class="form-group">
													<label class="text-muted">
														' . phrase('slider_thumbnail') . '
													</label>
													<div data-provides="fileupload" class="fileupload fileupload-new text-sm-center">
														<span class="btn btn-light btn-file btn-block">
															<input type="file" name="' . $field . '[thumbnail][]" accept="image/*" role="image-upload" id="' . $field . '_input" />
															<div class="fileupload-new text-center">
										...
										<div class="form-group">
											<input type="text" name="' . $field . '[title][]" class="form-control" placeholder="' . phrase('slider_title') . '" value="' . (isset($item['title']) ? $item['title'] : null) . '" id="' . $field . '_input" spellcheck="false" />
										</div>
										<div class="form-group">
											<textarea name="' . $field . '[description][]" class="form-control" placeholder="' . phrase('slider_description') . '" id="' . $field . '_input" rows="1" spellcheck="false">' . (isset($item['description']) ? $item['description'] : null) . '</textarea>
										</div>
										<div class="row">
											<div class="col-md-6">
												<div class="form-group">
													<input type="text" name="' . $field . '[link][]" class="form-control" placeholder="' . phrase('target_url') . '" value="' . (isset($item['link']) ? $item['link'] : null) . '" id="' . $field . '_input" spellcheck="false" />
												</div>
											</div>
											<div class="col-md-6">
												<div class="form-group">
													<input type="text" name="' . $field . '[label][]" class="form-control" placeholder="' . phrase('button_label') . '" value="' . (isset($item['label']) ? $item['label'] : null) . '" id="' . $field . '_input" spellcheck="false" />
												</div>
											</div>
										</div>
									</div>
									<div class="card-footer">
										<input type="hidden" name="' . $field . '[default_background][]" value="' . (isset($item['background']) ? $item['background'] : null) . '" />
										<input type="hidden" name="' . $field . '[default_thumbnail][]" value="' . (isset($item['thumbnail']) ? $item['thumbnail'] : null) . '" />
										<div class="btn-group btn-group-xs">
											<a href="javascript:void(0)" class="btn btn-secondary --move-up" data-element=".card" data-toggle="tooltip" title="' . phrase('move_up') . '">
												<i class="mdi mdi-arrow-collapse-up"></i>
											</a>
											<a href="javascript:void(0)" class="btn btn-secondary --move-down" data-element=".card" data-toggle="tooltip" title="' . phrase('move_down') . '">
												<i class="mdi mdi-arrow-collapse-down"></i>
											</a>
											<a href="javascript:void(0)" class="btn btn-secondary" role="remove-carousel" data-element=".card">
												<i class="mdi mdi-window-close" data-toggle="tooltip" title="' . phrase('remove') . '"></i>
											</a>
										</div>
									</div>
						...
						$items						= '
							<div class="card mb-3">
								<div class="card-body">
									...
									<div class="form-group">
										<input type="text" name="' . $field . '[title][]" class="form-control" placeholder="' . phrase('slider_title') . '" id="' . $field . '_input" spellcheck="false" />
									</div>
									<div class="form-group">
										<textarea name="' . $field . '[description][]" class="form-control" placeholder="' . phrase('slider_description') . '" id="' . $field . '_input" rows="1" spellcheck="false"></textarea>
									</div>
									<div class="row">
										<div class="col-md-6">
											<div class="form-group">
												<input type="text" name="' . $field . '[link][]" class="form-control" placeholder="' . phrase('target_url') . '" id="' . $field . '_input" spellcheck="false" />
											</div>
										</div>
										<div class="col-md-6">
											<div class="form-group">
												<input type="text" name="' . $field . '[label][]" class="form-control" placeholder="' . phrase('button_label') . '" id="' . $field . '_input" spellcheck="false" />
											</div>
										</div>
									</div>
								</div>
					...
					$content						= $items . '<a href="javascript:void(0)" class="btn btn-secondary btn-sm" role="add-carousel" data-field="' . $field . '" data-image-placeholder="' . get_image($this->_set_upload_path, 'placeholder.png', 'thumb') . '"><i class="mdi mdi-plus-circle-outline"></i>&nbsp;' . phrase('add') . '</a></div>';
				}
							...
							$items					.= '
								<div class="card mb-3">
									<div class="card-header p-2">
										<div class="input-group input-group-sm">
											...
											<div class="input-group-append">
												...
												<a href="javascript:void(0)" class="btn btn-secondary" role="remove-faq" data-element=".card">
													<i class="mdi mdi-window-close" data-toggle="tooltip" title="' . phrase('remove') . '"></i>
												</a>
											</div>
										</div>
									...
									<div class="card-body p-2">
										<textarea name="' . $field . '[answer][]" class="form-control" role="wysiwyg" placeholder="' . phrase('detailed_answer_for_above_question') . '" id="' . $field . '_input" rows="1" spellcheck="false">' . $item['answer'] . '</textarea>
									</div>
								</div>
							';
						}
					...
					else
					{
						$items						.= '
							<div class="card mb-3">
								<div class="card-header p-2">
									<div class="input-group input-group-sm">
										<input type="text" name="' . $field . '[question][]" class="form-control" placeholder="' . phrase('question') . '" id="' . $field . '_input" spellcheck="false" />
										<div class="input-group-append">
											<a href="javascript:void(0)" class="btn btn-secondary --move-up" data-element=".card" data-toggle="tooltip" title="' . phrase('move_up') . '">
												<i class="mdi mdi-arrow-collapse-up"></i>
											</a>
								...
								<div class="card-body p-2">
									<textarea name="' . $field . '[answer][]" class="form-control" role="wysiwyg" placeholder="' . phrase('detailed_answer_for_above_question') . '" id="' . $field . '_input" rows="1" spellcheck="false"></textarea>
								</div>
							</div>
						';
					}
					$content						= $items . '<a href="javascript:void(0)" class="btn btn-secondary btn-sm" role="add-faq" data-field="' . $field . '"><i class="mdi mdi-plus-circle-outline"></i>&nbsp;' . phrase('add') . '</a>';
				}
								...
								<div class="card mb-3">
									<div class="card-body p-2 relative">
										<div class="btn-group btn-group-xs absolute" style="top: 5px; right: 10px; z-index: 1">
											...
											<a href="javascript:void(0)" class="btn btn-secondary" role="remove-step" data-element=".card">
												<i class="mdi mdi-window-close" data-toggle="tooltip" title="' . phrase('remove') . '"></i>
											</a>
										</div>
										<textarea name="' . $field . '[]" class="form-control" role="wysiwyg" placeholder="' . phrase('add_step') . '" id="' . $field . '_input" rows="1" spellcheck="false">' . $item . '</textarea>
									</div>
								</div>
							';
						}
						...
						$items						.= '
							<div class="card mb-3">
								<div class="card-body p-2 relative">
									...
									<textarea name="' . $field . '[]" class="form-control" role="wysiwyg" placeholder="' . phrase('add_step') . '" id="' . $field . '_input" rows="1" spellcheck="false"></textarea>
								</div>
							</div>
						';
					}
					$content						= $items . '<a href="javascript:void(0)" class="btn btn-secondary btn-sm" role="add-step" data-field="' . $field . '"><i class="mdi mdi-plus-circle-outline"></i>&nbsp;' . phrase('add') . '</a>';
				}
				elseif(in_array('dropdown', $type) || in_array('checkbox', $type) || in_array('radio', $type))
				{
					...
					if(is_array($parameter) && sizeof($parameter) > 0)
					{
						foreach($parameter as $value => $label)
						{
							if(in_array('dropdown', $type))
							{
								$options			.= '<option value="' . $value . '"' . (in_array($value, array($content, $extra_params)) ? ' selected' : null) . '>' . $label . '</option>';
							}
							elseif(in_array('checkbox', $type))
							{
								$checker			= json_decode($content, true);
								if(!is_array($checker))
								{
									$checker		= array($content);
								}
								$options			.= '
									<label>
										<input type="checkbox" name="' . $field . '[]" value="' . $value . '"' . (in_array($value, $checker) ? ' checked' : null) . ' />
										&nbsp;
										' . $label . '
										&nbsp;
									</label>
							...
							elseif(in_array('radio', $type))
							{
								$options			.= '
									<label>
										<input type="radio" name="' . $field . '" value="' . $value . '"' . ($value == $content ? ' checked' : null) . ' />
										' . $label . '
										&nbsp;
									</label>
									' . $extra_params . '
					...
					{
						$content					= '
							<select name="' . $field . '" class="form-control' . $extra_class . '" placeholder="' . phrase('please_choose') . '" id="' . $field . '_input"' . ($attribute ? ' ' . $attribute : null) . $read_only . '>
								' . $options . '
							</select>
						';
					}
				...
				{
					$content						= '<textarea name="' . $field . '" class="form-control' . $extra_class . '" role="tagsinput" placeholder="' . phrase('separate_with_comma') . '" id="' . $field . '_input"' . ($attribute ? ' ' . $attribute : null) . $read_only . ' spellcheck="false">' . $content . '</textarea>';
				}
				elseif(in_array('hour', $type))
				{
					...
					{
						$options					.= '<option value="' . $h . '"' . ($h == $original ? ' selected' : null) . '>' . sprintf('%02d', (24 == $h ? '00' : $h)) . ':00</option>';
					}
					$content						= '
						<select name="' . $field . '" class="form-control' . $extra_class . '" placeholder="' . phrase('choose_hour') . '" id="' . $field . '_input"' . ($attribute ? ' ' . $attribute : null) . $read_only . '>
							' . $options . '
						</select>
					';
				}
				elseif(in_array('date_only', $type))
				{
					...
					{
						$options					.= '<option value="' . $d . '"' . ($d == $original ? ' selected' : null) . '>' . sprintf('%02d', $d) . '</option>';
					}
					$content						= '
						<select name="' . $field . '" class="form-control' . $extra_class . '" placeholder="' . phrase('choose_date') . '" id="' . $field . '_input"' . ($attribute ? ' ' . $attribute : null) . $read_only . '>
							' . $options . '
						</select>
					';
				}
				...
				{
					$content						= '<input type="text" name="' . $field . '" class="form-control' . $extra_class . '" role="datepicker" data-modal="true" data-large-mode="true" placeholder="' . phrase('click_to_select_data') . '" value="' . ($content && $content != '0000-00-00' ? date('d M Y', strtotime($content)) : date('d M Y')) . '" id="' . $field . '_input" maxlength="' . $max_length . '" readonly spellcheck="false" />';
				}
				elseif(in_array('datetime', $type))
				{
					$content						= '<input type="text" name="' . $field . '" class="form-control' . $extra_class . '" role="datetime" data-modal="true" data-large-mode="true" placeholder="' . phrase('click_to_select_data') . '" value="' . ($content && $content != '0000-00-00 00:00:00' ? $content : date('Y-m-d H:i:s')) . '" id="' . $field . '_input" maxlength="' . $max_length . '" readonly spellcheck="false" />';
				}
				elseif(in_array('monthpicker', $type))
				{
					...
					{
						$curmonth					= explode('-', $content);
						$n							= sprintf('%02d', $n);
						$options					.= '<option value="' . $n . '"' . (isset($curmonth[1]) && $curmonth[1] == $n ? ' selected' : null) . '>' . phrase(date('F', mktime(0, 0, 0, $n, 10))) . '</option>';
					}
					$content						= '
						<select name="' . $field . '" class="form-control' . $extra_class . '" placeholder="' . phrase('choose_month') . '" id="' . $field . '_input"' . ($attribute ? ' ' . $attribute : null) . $read_only . '>
							' . $options . '
						</select>
					';
				}
				elseif(in_array('yearpicker', $type))
				{
					...
					foreach($year as $k => $y)
					{
						$options					.= '<option value="' . $y . '"' . ($original == $y ? ' selected' : null) . '>' . $y . '</option>';
					}
					$content						= '
						<select name="' . $field . '" class="form-control' . $extra_class . '" placeholder="' . phrase('choose_year') . '" id="' . $field . '_input"' . ($attribute ? ' ' . $attribute : null) . $read_only . '>
							' . $options . '
						</select>
					';
				}
				elseif(in_array('quarterly', $type))
				{
					...
					$content						= '
						<select name="' . $field . '" class="form-control ' . $extra_class . '" id="' . $field . '_input"' . ($attribute ? ' ' . $attribute : null) . $read_only . '>
							<option value="" readonly>' . phrase('please_choose') . '</option>
							<option value="' . $year . '-01-01"' . ($content == $year . '-01-01' ? ' selected' : null) . '>' . phrase('quarter') . ' I</option>
							<option value="' . $year . '-04-01"' . ($content == $year . '-04-01' ? ' selected' : null) . '>' . phrase('quarter') . ' II</option>
							<option value="' . $year . '-07-01"' . ($content == $year . '-07-01' ? ' selected' : null) . '>' . phrase('quarter') . ' III</option>
							<option value="' . $year . '-10-01"' . ($content == $year . '-10-01' ? ' selected' : null) . '>' . phrase('quarter') . ' IV</option>
						</select>
					';
				}
				elseif(in_array('wysiwyg', $type))
				{
					$content						= '<textarea name="' . $field . '" class="form-control' . $extra_class . '" role="wysiwyg" data-upload-path="' . $this->_set_upload_path . '" placeholder="' . phrase('type_the_content_here') . '" id="' . $field . '_input"' . $attribute . ' maxlength="' . $max_length . '" rows="1"' . ($attribute ? ' ' . $attribute : null) . $read_only . ' spellcheck="false">' . $content . '</textarea>';
				}
				elseif(in_array('textarea', $type))
				{
					$content						= '<textarea name="' . $field . '" class="form-control' . (in_array('autocomplete', $type) && $extra_params ? ' on-autocomplete-trigger' : null) . $extra_class . '" id="' . $field . '_input"' . $attribute . ' maxlength="' . $max_length . '" rows="1"' . (in_array('autocomplete', $type) ? ' role="autocomplete" data-href="' . current_page(null, array('autocomplete_table' => $parameter, 'autocomplete_field' => $extra_params)) . '"': '') . ($attribute ? ' ' . $attribute : null) . $read_only . ' spellcheck="false">' . ($default_value ? $default_value : $content) . '</textarea>';
				}
				elseif(in_array('price_format', $type))
				{
					$content						= '<input type="text" name="' . $field . '" min="0" class="form-control text-right' . $extra_class . '" value="' . ($content ? $content : 0) . '" role="price" placeholder="' . phrase('number_only') . '" id="' . $field . '_input" maxlength="' . $max_length . '"' . ($attribute ? ' ' . $attribute : null) . $read_only . ' spellcheck="false" />';
				}
				elseif(in_array('number_format', $type) || in_array('percent_format', $type))
				{
					$content						= '<input type="number" name="' . $field . '" min="0" class="form-control' . $extra_class . '" value="' . (is_numeric($original) ? number_format($original, (in_array('percent_format', $type) ? 2 : 0), '.', '') : 0) . '" placeholder="' . phrase('number_only') . '" id="' . $field . '_input" maxlength="' . $max_length . '"' . (is_numeric($parameter) || in_array('percent_format', $type) ? ' pattern="[0-9]+([\.,][0-9]+)?" step="0.01"' : '') . ($attribute ? ' ' . $attribute : null) . $read_only . ' />';
				...
				elseif(in_array('boolean', $type))
				{
					$content						= '
						<label class="d-block">
							<input type="checkbox" name="' . $field . '" value="1" id="' . $field . '_input"' . (1 == $content || 'create' == $this->_method ? ' checked' : null) . ' />
							&nbsp;
				...
				{
					$content						= '<input type="email" name="' . $field . '" class="form-control' . $extra_class . '" placeholder="' . phrase('type_the_valid_email') . '" value="' . ($default_value ? $default_value : $content) . '" id="' . $field . '_input" maxlength="' . $max_length . '"' . ($attribute ? ' ' . $attribute : null) . $read_only . ' />';
				}
				...
				{
					$content						= '<input type="hidden" name="' . $field . '" value="' . $content . '" />';
				}
				...
				elseif(in_array('last_insert', $type))
				{
					...
					if('create' == $this->_method)
					{
						...
						if($this->_where)
						{
							foreach($this->_where as $w_key => $w_val)
							{
								if($this->model->field_exists($w_key, $this->_from))
								{
									$where[$w_key]	= $w_val;
								}
							}
						...
						$content					= ($last_insert > 0 ? $last_insert : 1);
						if(in_array('sprintf', $type))
						{
							$content				= sprintf((is_string($extra_params) ? $extra_params : '%04d'), $content);
						}
						if($parameter)
						{
							$content				= str_replace('{1}', $content, $parameter);
						}
					}
					else
					{
					...
					$content						= '<input type="text" name="' . $field . '" class="form-control' . $extra_class . '" value="' . $content . '" id="' . $field . '_input" maxlength="' . $max_length . '"' . ($attribute ? ' ' . $attribute : null) . $read_only . ' spellcheck="false" />';
				}
				elseif(in_array('to_slug', $type))
				{
					$content						= '<input type="text" name="' . $field . '" class="form-control' . $extra_class . '" value="' . $content . '" placeholder="' . (!$read_only ? phrase('leave_blank_to_generate_automatically') : null) . '" id="' . $field . '_input" maxlength="' . $max_length . '"' . ($attribute ? ' ' . $attribute : null) . $read_only . ' spellcheck="false" />';
				}
				elseif(in_array('coordinate', $type))
				{
					$paths							= ($original && is_json($original) ? json_decode($original) : null);
					$colorscheme					= (isset($paths->colorscheme) ? $paths->colorscheme : '#FF0000');
					$content						= '
						<div class="drawing-placeholder preloader w-100" style="position:relative">
							' . ('google' == get_setting('maps_provider') ? '<input type="text" class="form-control" id="place-search" placeholder="' . phrase('search_a_place') . '" autocomplete="off" style="position:absolute; width: 240px; margin:10px; z-index:1" /><div class="input-group-sm" role="colorpicker" style="position:absolute; top:14px; left: 216px; width:auto; z-index:1"><input type="hidden" class="colorscheme" value="' . $colorscheme . '" /><div class="input-group-append" data-toggle="tooltip" title="' . phrase('change_marker_color') . '"><span class="input-group-text" style="width: 30px; border:none; border-radius:50%">&nbsp;</span></div></div>' : null) . '
							<div id="map_' . $field . rand(0, 999) . '"' . ($extra_class ? ' class="' . $extra_class . '"' : null) . ' role="map" data-coordinate="' . strip_tags(htmlspecialchars(($original && is_json($original) ? $original : get_setting('office_map')))) . '" data-apply-coordinate-to="#' . $field . '_input" data-apply-address-to=".map-address-listener" data-draggable="1" data-geocoder="1"' . (!$original ? ' data-geolocation="1"' : null) . ' data-mousewheel="0" title="' . phrase('drag_marker_to_update_location') . '"' . (isset($this->_add_attribute[$field]) ? ' ' . $this->_add_attribute[$field] : null) . ' style="height:260px"></div>
							<input type="hidden" name="' . $field . '" id="' . $field . '_input" value="' . strip_tags(htmlspecialchars(($original && is_json($original) ? $original : get_setting('office_map')))) . '" />
						</div>
					';
				}
				elseif(in_array('polygon', $type))
				{
					...
					$content						= '
						<div class="drawing-placeholder preloader w-100" style="position:relative">
							' . ('google' == get_setting('maps_provider') ? '<input type="text" class="form-control" id="place-search" placeholder="' . phrase('search_a_place') . '" autocomplete="off" style="position:absolute; width: 240px; margin:10px; z-index:1" /><div class="input-group-sm" role="colorpicker" style="position:absolute; top:14px; left: 216px; width:auto; z-index:1"><input type="hidden" class="colorscheme" value="' . $colorscheme . '" /><div class="input-group-append" data-toggle="tooltip" title="' . phrase('change_polygon_color') . '"><span class="input-group-text" style="width: 30px; border:none; border-radius:50%">&nbsp;</span></div></div>' : null) . '
							<div id="map_' . $field . rand(0, 999) . '"' . ($extra_class ? ' class="' . $extra_class . '"' : null) . ' role="map" data-coordinate="' . strip_tags(htmlspecialchars(get_setting('office_map'))) . '" data-polygon="' . strip_tags(htmlspecialchars(($original && is_json($original) ? $original : null))) . '" data-apply-coordinate-to="#' . $field . '_input"' . (in_array($this->_method, array('create', 'update')) ? ' data-drawing-manager="1" data-drawing-type="polygon"' : null) . ' data-geocoder="1"' . (!$original ? ' data-geolocation="1"' : null) . ' data-mousewheel="0" title="' . phrase('click_map_to_start_drawing') . '"' . (isset($this->_add_attribute[$field]) ? ' ' . $this->_add_attribute[$field] : null) . ' style="height:360px"></div>
							<input type="hidden" name="' . $field . '" class="polygon-path" id="' . $field . '_input" value="' . strip_tags(htmlspecialchars(($original && is_json($original) ? $original : '[]'))) . '" />
						</div>
					';
				}
				elseif(in_array('linestring', $type))
				{
					...
					$content						= '
						<div class="drawing-placeholder preloader w-100" style="position:relative">
							' . ('google' == get_setting('maps_provider') ? '<input type="text" class="form-control" id="place-search" placeholder="' . phrase('search_a_place') . '" autocomplete="off" style="position:absolute; width: 240px; margin:10px; z-index:1" /><div class="input-group-sm" role="colorpicker" style="position:absolute; top:14px; left: 216px; width:auto; z-index:1"><input type="hidden" class="colorscheme" value="' . $colorscheme . '" /><div class="input-group-append" data-toggle="tooltip" title="' . phrase('change_polyline_color') . '"><span class="input-group-text" style="width: 30px; border:none; border-radius:50%">&nbsp;</span></div></div>' : null) . '
							<div id="map_' . $field . rand(0, 999) . '"' . ($extra_class ? ' class="' . $extra_class . '"' : null) . ' role="map" data-coordinate="' . strip_tags(htmlspecialchars(get_setting('office_map'))) . '" data-linestring="' . strip_tags(htmlspecialchars(($original && is_json($original) ? $original : null))) . '" data-apply-coordinate-to="#' . $field . '_input"' . (in_array($this->_method, array('create', 'update')) ? ' data-drawing-manager="1" data-drawing-type="linestring"' : null) . ' data-geocoder="1"' . (!$original ? ' data-geolocation="1"' : null) . ' data-mousewheel="0" title="' . phrase('click_map_to_start_drawing') . '"' . (isset($this->_add_attribute[$field]) ? ' ' . $this->_add_attribute[$field] : null) . ' style="height:360px"></div>
							<input type="hidden" name="' . $field . '" class="linestring-path" id="' . $field . '_input"' . $attribute . ' value="' . strip_tags(htmlspecialchars(($original && is_json($original) ? $original : '[]'))) . '" />
						</div>
					';
				}
				else
				{
					$content						= '<input type="text" name="' . $field . '" class="form-control' . (in_array('autocomplete', $type) && $extra_params ? ' on-autocomplete-trigger' : null) . $extra_class . '" value="' . ($default_value ? $default_value : $content) . '" id="' . $field . '_input" maxlength="' . $max_length . '"' . ($attribute ? ' ' . $attribute : null) . (in_array('autocomplete', $type) && $extra_params ? ' role="autocomplete" data-href="' . current_page() . '"': '') . $read_only . ' spellcheck="false" />';
				}
	...
	public function render_read($data = array())
	{
		...
		if($serialized)
		{
			...
			foreach($serialized as $field => $params)
			{
				...
				elseif(in_array('custom_format', $type))
				{
					if(method_exists($this, str_replace('callback_', null, $parameter)))
					{
				...
				elseif(in_array('image', $type))
				{
					$content						= '
						<div class="text-center">
							<a href="' . get_image($this->_set_upload_path, $content) . '" target="_blank">
								<img src="' . ($original ? get_image($this->_set_upload_path, $original, 'thumb') : $parameter) . '" alt="' . ($original ? $original : $parameter) . '" class="img-fluid rounded" />
							</a>
						</div>
					';
				}
				...
				elseif(in_array('carousels', $type))
				{
					...
					if(is_array($content) && sizeof($content) > 0)
					{
						foreach($content as $slideshow => $value)
						{
							$items					.= '
				...
				elseif(in_array('date', $type) || in_array('datepicker', $type))
				{
					...
					$month							= date('F', $timestamp);
					$month							= phrase($month);
					$content						= '<span class="badge badge-light">' . date('d', $timestamp) . ' ' . $month . ' ' . date('Y', $timestamp) . '</span>';
				}
				elseif(in_array('monthpicker', $type))
				{
				...
				elseif(in_array('dropdown', $type) || in_array('checkbox', $type) || in_array('radio', $type))
				{
					...
					elseif(in_array('checkbox', $type))
					{
						...
						if(is_array($json) && sizeof($json) > 0)
						{
							...
							{
								if(isset($this->_set_field[$field]['parameter'][$val]))
								{
				...
				elseif(in_array('coordinate', $type))
				{
					$content						= '
						<div class="drawing-placeholder preloader w-100" style="position:relative">
							<div id="' . $field . rand(0, 999) . '"' . ($extra_class ? ' class="' . $extra_class . '"' : null) . ' role="map" data-draggable="false" data-coordinate="' . strip_tags(htmlspecialchars(($original && is_json($original) ? $original : get_setting('office_map')))) . '" style="height:260px"></div>
						</div>
					';
				}
				elseif(in_array('polygon', $type))
				{
					$content						= '
						<div class="drawing-placeholder preloader w-100" style="position:relative">
							<div id="' . $field . rand(0, 999) . '"' . ($extra_class ? ' class="' . $extra_class . '"' : null) . ' role="map" data-coordinate="' . strip_tags(htmlspecialchars(get_setting('office_map'))) . '" data-polygon="' . strip_tags(htmlspecialchars(($original && is_json($original) ? $original : '[]'))) . '" style="height:260px"></div>
						</div>
					';
				}
				elseif(in_array('linestring', $type))
				{
					$content						= '
						<div class="drawing-placeholder preloader w-100" style="position:relative">
							<div id="' . $field . rand(0, 999) . '"' . ($extra_class ? ' class="' . $extra_class . '"' : null) . ' role="map" data-coordinate="' . strip_tags(htmlspecialchars(get_setting('office_map'))) . '" data-linestring="' . strip_tags(htmlspecialchars(($original && is_json($original) ? $original : '[]'))) . '" style="height:260px"></div>
						</div>
					';
				}
				else
			...
			foreach($serialized as $key => $val)
			{
				...
				foreach($val as $field => $params)
				{
					...
					elseif(in_array('custom_format', $type))
					{
						...
						{
							$callback				= str_replace('callback_', null, $parameter);
							$content				= $this->$callback($custom_format);
						}
						else
						{
							$content				= $parameter;
					...
					elseif($original && (in_array('datetime', $type) || in_array('current_timestamp', $type)))
					{
						...
						$month						= date('F', $timestamp);
						$month						= phrase($month);
						$content					= '<span class="badge badge-light" data-toggle="tooltip" title="' . date('d', $timestamp) . ' ' . $month . ' ' . date('Y - H:i:s', $timestamp) . '">' . date('d', $timestamp) . ' ' . $month . ' ' . date('Y', $timestamp) . '</span>';
					}
					elseif(in_array('hour', $type))
					{
						$content					= sprintf('%02d', (24 == $original ? '00' : $original)) . ':00';
					...
					else
					{
						...
						{
							$content				= (!in_array($field, $this->_unset_truncate) ? truncate($content, 60) : $content);
						}
									...
									$content		= '
										<a href="' . base_url((is_array($this->_set_field[$field]['parameter']) && sizeof($this->_set_field[$field]['parameter']) > 1 ? $this->_set_field[$field]['parameter'][$hyperlink_params] : $this->_set_field[$field]['parameter']), $uri) . '"' . ('_blank' == $another_params ? ' target="_blank"' : ' class="' . ($another_params ? $another_params : '--xhr') . '"') . ' style="display:block">
											<b data-toggle="tooltip" title="' . phrase('click_to_open') . '">
												' . ($content ? $content : $this->_set_field[$field]['placeholder']) . '
											</b>
										</a>
									';
								}
				...
				$token								= generate_token(array_filter(array_merge($query_string, $primary_key)));
				$document_token						= generate_token(array_filter(array_merge($query_string, $primary_key, array('r' => 'document'))));
				$security_token[]					= array
				(
					'hash'							=> $token,
			...
			foreach($search_columns as $key => $val)
			{
				...
				if(!in_array($val, $this->_unset_column))
				{
					...
					(
						...
						'token'						=> generate_token(($query_string ? array_merge($query_string, array('order' => $val, 'sort' => get_userdata('sortOrder'))) : array('order' => $val, 'sort' => get_userdata('sortOrder')))),
		...
		if($data)
		{
			...
			foreach($data as $row => $array)
			{
				foreach($array as $key => $content)
				{
					...
					elseif(('read' == $this->_method || (in_array($this->_method, array('print', 'pdf')) && 'document' == $this->input->get('r'))) && in_array($key, $this->_unset_view))
					{
						$hidden						= true;
					}
	...
	public function validate_form($data = array())
	{
		...
		if($this->_restrict_on_demo)
		{
			return throw_exception(301, phrase('data_has_been_successfully_submitted_and_will_not_saved_in_demo_mode'), $this->_redirect_back);
		}
	...
	private function _push_log()
	{
		...
		elseif(file_put_contents(LOG_PATH . '/' . $filename, json_encode($source)))
		{
			// log recorded
		}
	}
