 ...
 */
function utm_to_coordinate($x = 0, $y = 0, $zone = 0, $aboveEquator = false)
{
	...
	(
		array
		(
			radian_to_degree($latlon[1]),
			radian_to_degree($latlon[0])
		)
	);
}
 ...
 */
function coordinate_to_utm($lat = 0, $lon = 0)
{
	if(!is_numeric($lon))
	{
		return json_encode
	...
	if($lon <- 180.0 || $lon >= 180.0)
	{
		...
		(
			...
			(
				'success'							=> false,
				'message'							=> 'The longitude is out of range'
			)
		);
	}
