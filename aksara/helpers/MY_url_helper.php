...
if (!function_exists('base_url'))
{
	...
	function base_url($method = '', $params = array())
	{
		...
		if(is_array($params) && sizeof($params) > 0)
		{
			...
			$token									= $q;
			unset($token['token']);
			$q['token']								= generate_token($token);
			$uri									= $method . '?' . http_build_query($q);
		}
		else
		{
			$uri									= $method;
...
if(!function_exists('current_page'))
{
	...
	function current_page($method = null, $params = array(), $unset = null)
	{
		...
		if(is_array($params) && sizeof($params) > 0)
		{
			...
			unset($token['token']);
			$q['token']								= generate_token($token);
			return $output . $method . '?' . http_build_query($q);
		}
		else
		{
...
if(!function_exists('go_to'))
{
	...
	function go_to($method = null, $params = array())
	{
		...
		if(is_array($params) && sizeof($params) > 0)
		{
			...
			unset($token['token']);
			$q['token']								= generate_token($token);
			$uri									= $CI->_slug . '/' . $method . '?' . http_build_query($q);
		}
		else
		{
			$uri									= $CI->_slug . '/' . $method;
