...
class Connector
{
	...
	public function connect($driver = array(), $hostname = null, $port = 0, $username = null, $password = null, $database = null)
	{
		...
		{
			$this->_parameter['dsn']				= 'dblib:host=' . $this->_parameter['hostname'] . ($this->_parameter['port'] ? ':' . $this->_parameter['port'] : '') . ';dbname=' . $this->_parameter['database'];
			
			$try									= new PDO('dblib:host=' . $this->_parameter['hostname'] . ($this->_parameter['port'] ? ':' . $this->_parameter['port'] : '') . ';dbname=' . $this->_parameter['database'], $this->_parameter['username'], $this->_parameter['password']);
			if(!$try)
			{
				$error								= true;
			}
