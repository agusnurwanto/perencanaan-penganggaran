...
class Template
{
	...
	public function render($view = null, $data = array(), $breadcrumb = array(), $language = null)
	{
		...
		// Generate output
		$title									= (isset($data->title) ? $data->title : phrase('title_was_not_set'));
		$description							= (isset($data->description) ? $data->description : null);
		$icon									= (isset($data->icon) ? $data->icon : 'mdi mdi-table');
		
		/* check if request is from Promise */
		if($this->_ci->input->is_ajax_request())
		{
			/* format output to html */
			$data->html							= trim(preg_replace('/(?!<pre[^>]*?>)[\t\n\r\s]+(?![^<]*?<\/pre>)/i', ' ', $this->_ci->load->view($view, $data, true)));
			
			/* remove all predefined object that have been passing and processing in the html */
			foreach($data as $key => $val)
			{
				/* only matching key will keep */
				if(!in_array($key, array('breadcrumb', 'current_page', 'html', 'meta', 'method', 'pagination', 'query_string', 'token')))
				{
					/* key not match, remove from object */
					unset($data->$key);
				}
			}
		...
		else
		{
			...
			$this->title($data->meta->title)->build($view, $data);
		}
	}
			...
			if($segment && $label)
			{
				if($segment != $current_slug)
				{
					...
					(
						...
						'label'					=> ($found ? phrase($label) : ucwords(preg_replace('!\s+!', '_', trim(preg_replace('/([^0-9a-z])/i', ' ', $label))))),
						'icon'					=> null
					);
				}
			}
				...
				<div class="row">
					...
					<div class="col-sm-6">
						<nav class="d-flex justify-content-center justify-content-sm-end justify-content-md-end justify-content-lg-end justify-content-xl-end" aria-label="Page navigation">
							...
							<form action="' . current_page(null, array('per_page' => null)) . '" method="GET" class="--xhr-form ml-2">
								' . $query_string . '
